...
});return k}function init_device(){var a=$("#activityIndicator");log("init_device()"),deviceVars.scrollProperty="-moz-transform",deviceVars.scrollValue="translateY($1px)",$navBar=$("#navBoxHeader"),activityIndicatorTop=Math.floor($(window).height()/2),a.css("top",activityIndicatorTop),$("#startUp-initDevice").addClass("success")
}function isCameraPresent(){return isBlinkGapDevice.hasCamera()}function triggerScroll(){$(window).trigger("scroll")
...
},m.options&&m.options.delayInit||m.init()}(window);var activityIndicatorTop,$navBar,onDeviceReady=$.noop;
...
return MyAnswers.dispatch.add(function(){c.hide(),c.css({"z-index":0,position:"absolute"}),e.css({"z-index":50,position:"absolute"}),currentMasterCategory?c.attr({"data-name":siteVars.config["m"+currentMasterCategory].pertinent.name}):b("#categoriesView").removeAttr("data-name"),currentCategory?c.attr({"data-name":siteVars.config["c"+currentCategory].pertinent.name}):b("#keywordListView").removeAttr("data-name"),currentInteraction?c.attr({"data-name":siteVars.config["i"+currentInteraction].pertinent.name,"data-type":siteVars.config["i"+currentInteraction].pertinent.type}):(b("#answerView").removeAttr("data-name"),b("#answerView").removeAttr("data-type")),c.show(),"screen-bottom"!==a.currentConfig.footerPosition&&MyAnswers.$body.children("footer").hide(),f.find("button").prop("disabled",!0),d.resolve()
}),d.promise()},c.showView=function(a,d){var e=new b.Deferred,f=b(".view:visible").not(a[0]),g=d?"right":"left";
return c.hideLocationBar(),MyAnswers.dispatch.add(function(){0!==f.length?f.hide("slide",{direction:g},300,function(){f.css("z-index",""),f.css("position",""),a.css("z-index",""),a.css("position",""),updateNavigationButtons(),MyAnswers.$body.children("footer").show(),e.resolve(),a.trigger("viewShow")
}):(a.css("z-index",""),a.css("position",""),updateNavigationButtons(),MyAnswers.$body.children("footer").show(),e.resolve(),a.trigger("viewShow"))
}),e.promise()},c};a.MyAnswersDevice=new c}(this),document.getElementById("startUp-loadDevice").className="working success",MyAnswers.deviceDeferred.resolve(),function(a){"use strict";
...
//# sourceMappingURL=gecko.min.map