...
});return k}function init_device(){var a=$("#activityIndicator");log("init_device()"),deviceVars.scrollProperty="-webkit-transform",deviceVars.scrollValue="translateY($1px)",$navBar=$("#navBoxHeader"),activityIndicatorTop=Math.floor($(window).height()/2),a.css("top",activityIndicatorTop),$("#startUp-initDevice").addClass("success")
...
}catch(a){log("onDeviceReady exception: "),log(a)}}function isCameraPresent(){return isBlinkGapDevice.hasCamera()
}function triggerScroll(){$(window).trigger("scroll")}function insertHTML(a,b){"object"===$.type(a)&&MyAnswers.dispatch.add(function(){$(a).html(b),$("img").off("error",siteVars.fixImages),$("img").on("error",siteVars.fixImages)
...
c=parseInt(c,10),"number"!=typeof c||isNaN(c)||(d._lc=c),deviceVars.isOnline?$.ajax({url:"/_R_/xhr/GetMoJO.php",data:d,dataType:"xml",complete:function(c){200===c.status||0===c.status?$.when(MyAnswers.store.set("mojoXML:"+a,c.responseText)).fail(b.reject).then(b.resolve):b.resolve()
...
c&&("LOGGED IN"===c.status?(MyAnswers.loginAccount=c,MyAnswers.isLoggedIn=!0,$.when(window.requestConfig()).always(function(){siteVars.map&&siteVars.config?window.updateNavigationButtons():BMP.alert("error: insufficient data, check network and reload / refresh")
...
return currentConfig.loginAccess&&currentConfig.loginUseInteractions&&f===e&&(b=MyAnswers.isLoggedIn,$.when(requestLoginStatus()).always(function(){MyAnswers.isLoggedIn!==b&&(MyAnswers.isLoggedIn?$.when(window.requestConfig()).always(function(){siteVars.map&&siteVars.config?(window.updateNavigationButtons(),currentConfig.loginToDefaultScreen&&MyAnswers.gotoDefaultScreen()):BMP.alert("error: insufficient data, check network and reload / refresh")
...
200!==a.status&&0!==a.status||"string"!=typeof a.responseText||(a.responseText=a.responseText.substring(a.responseText.indexOf("<formObjects>")),b=$($.parseXML(a.responseText)),b.children().children().each(g)),200===a.status||304===a.status||0===a.status?c.resolve():(warn("processForms()->GetForm.XHR: failed "+a.status),c.reject())
},timeout:Math.max(1e3*currentConfig.downloadTimeout,computeTimeout(512e3))}):window.cordova&&window.cordova.offline?cordova.offline.retrieveContent(function(a){var b;
a=a||"",a=a.substring(a.indexOf("<formObjects>"));try{b=$($.parseXML(a)),b.children().children().each(g),c.resolve()
}catch(d){error("processForms()->GetForm.Retrieve: failed ",d),c.reject()}},function(){warn("processForms()->GetForm.Retrieve: failed "),c.reject()
},{url:"/_R_/xhr-forms/GetForm.php"}):c.resolve()),$.whenArray(a).fail(MyAnswers.formsDeferred.reject).then(MyAnswers.formsDeferred.resolve)
}function processConfig(a){var b,c,d,e;d=MyAnswers.device.persistentStorage,a&&"object"==typeof a&&(e=["a"+siteVars.id],"object"===$.type(a.map)&&(siteVars.map=a.map,d&&MyAnswers.siteStore.set("map",JSON.stringify(siteVars.map))),"object"===$.type(a["a"+siteVars.id])&&(b=a["a"+siteVars.id].pertinent.siteStructure),siteVars.map&&("master categories"===b&&siteVars.map.masterCategories.length>0&&(e=e.concat($.map(siteVars.map.masterCategories,function(a){return"m"+a
}))),"interactions only"!==b&&siteVars.map.categories.length>0&&(e=e.concat($.map(siteVars.map.categories,function(a){return"c"+a
}))),siteVars.map.interactions.length>0&&(e=e.concat($.map(siteVars.map.interactions,function(a){return"i"+a
})))),("object"!==$.type(siteVars.config)||e.length>0)&&(siteVars.config={}),$.each(e,function(b,c){"object"===$.type(a[c])&&(siteVars.config[c]=a[c])
}),d&&MyAnswers.siteStore.set("config",JSON.stringify(siteVars.config)),deviceVars.features=a.deviceFeatures),siteVars.config&&siteVars.map&&(MyAnswers.isEmptySpace=!1,MyAnswers.isLoginOnly=!1,log("processConfig(): currentMasterCategory="+currentMasterCategory+" currentCategory="+currentCategory+" currentInteraction="+currentInteraction),"object"===$.type(siteVars.config["a"+siteVars.id])?(c=siteVars.config["a"+siteVars.id].pertinent,b=c.siteStructure,"master categories"===b&&(hasMasterCategories=siteVars.map.masterCategories.length>0),"interactions only"!==b&&(hasCategories=siteVars.map.categories.length>0),hasInteractions=siteVars.map.interactions.length>0,answerSpaceOneKeyword=1===siteVars.map.interactions.length,c.loginAccess&&c.loginUseInteractions&&c.loginPromptInteraction&&c.loginStatusInteraction&&siteVars.config["i"+c.loginPromptInteraction]&&siteVars.config["i"+c.loginStatusInteraction]&&(MyAnswers.isCustomLogin=!0),siteVars.map&&0===siteVars.map.masterCategories.length&&0===siteVars.map.categories.length&&siteVars.map.interactions.length===(MyAnswers.isCustomLogin?2:0)&&(MyAnswers.isEmptySpace=!0),c.loginAccess&&!MyAnswers.isLoggedIn&&("deny anonymous"===c.registeredOnly||MyAnswers.isEmptySpace)&&(MyAnswers.isLoginOnly=!0)):log("processConfig(): unable to retrieve answerSpace config"),processForms())
}function requestConfig(){var a,b,c;return a=new $.Deferred,b="/_R_/xhr/GetConfig.php",b+="?_asn="+siteVars.answerSpace,MyAnswers.isLoggedIn&&"object"===$.type(MyAnswers.loginAccount)&&(b+="&_username="+MyAnswers.loginAccount.username),c=function(){return MyAnswers.isLoggedIn||!window.applicationCache?!1:applicationCache.status===applicationCache.IDLE
}(),deviceVars.isOnline||c?$.ajax({url:b,type:"GET",dataType:"json",timeout:computeTimeout(40960),complete:function(b){var c;
if(200===b.status||0===b.status)try{c=$.parseJSON(b.responseText)}catch(d){warn("requestConfig.parse: failed"),warn(d)
}processConfig(c),200===b.status||304===b.status||0===b.status?a.resolve():(warn("requestConfig.ajax: did not succeed",b.status),a.reject())
}}):window.cordova&&window.cordova.offline?cordova.offline.retrieveContent(function(b){try{b=$.parseJSON(b)
}catch(c){warn(c)}processConfig(b),a.resolve()},function(b){warn("requestConfig.retrieve: failed",b),processConfig(),a.reject(b)
},{url:b}):(warn("requestConfig: no network, AppCache or Cordova"),processConfig(),a.reject()),a.promise()
}function gotoStorageView(){var a,b;if(currentConfig.activeFormsUseInteraction){if(a=siteVars.config["i"+currentConfig.activeFormsInteraction],"object"!==$.type(a))return void BMP.alert("Interaction for active forms listing could not be found.");
...
}(this),function(a){"use strict";var b=a.jQuery;a.isBlinkGapDevice=function(){return a.cordova&&a.cordova.offline?!0:a.PhoneGap&&"object"===b.type(a.device)&&a.device instanceof a.Device
},a.isBlinkGapDevice.hasCamera=function(){return a.isBlinkGapDevice()&&!!(a.Camera||navigator.camera||{}).getPicture
},a.isBlinkGapDevice.hasTouchDraw=function(){return a.isBlinkGapDevice()&&!!(a.BGTouchDraw||navigator.bgtouchdraw||{}).getDrawing
},a.computeTimeout=function(a){var b=1e3/600;return Math.floor(a*b+15e3)}}(this),function(a){"use strict";
var b,c,d,e,f,g;b=a.jQuery,c=b(a.document),d={history:[]},f=["log","error","warn","info"],g=function(){c.off("ready deviceready",g),setTimeout(function(){e=a.console||a.debug||{log:b.noop},b.each(f,function(c,d){var f=b.type(e[d]);
a[d]="function"===f?function(){e[d].apply(e,arguments)}:"object"===f?function(a){e[d](a)
}:"log"!==d?a.log:b.noop}),b.each(d.history,function(c,d){var f=a[d.fn],g=b.type(f);
"function"===g?f.apply(e,d.args):"object"===g&&f(d.args[0])}),delete d.history,b.each(f,function(a,b){delete d[b]
})},0)},e=d,b.each(f,function(c,f){d[f]=function(){d.history.push({fn:f,args:b.makeArray(arguments)})
},a[f]=e[f]}),a.isBlinkGap||a.isBlinkGapDevice()?a.BMP.waitForBlinkGap().then(function(){c.ready(g)
},function(){c.ready(g)}):c.ready(g)}(this),function(a){"use strict";var b=a.String;
...
}catch(j){error(j),e=!1}return e},a._Blink.cdnp=new g}(this),function(){"use strict";
function a(a){throw a}function b(b,c,d){4!==c.length&&a(new k.exception.invalid("invalid aes block size"));
var e=b.b[d],f=c[0]^e[0],g=c[d?3:1]^e[1],h=c[2]^e[2];c=c[d?1:3]^e[3];var i,j,l,m,n=e.length/4-2,o=4,p=[0,0,0,0];
i=b.k[d],b=i[0];var q=i[1],r=i[2],s=i[3],t=i[4];for(m=0;n>m;m++)i=b[f>>>24]^q[g>>16&255]^r[h>>8&255]^s[255&c]^e[o],j=b[g>>>24]^q[h>>16&255]^r[c>>8&255]^s[255&f]^e[o+1],l=b[h>>>24]^q[c>>16&255]^r[f>>8&255]^s[255&g]^e[o+2],c=b[c>>>24]^q[f>>16&255]^r[g>>8&255]^s[255&h]^e[o+3],o+=4,f=i,g=j,h=l;
for(m=0;4>m;m++)p[d?3&-m:m]=t[f>>>24]<<24^t[g>>16&255]<<16^t[h>>8&255]<<8^t[255&c]^e[o++],i=f,f=g,g=h,h=c,c=i;
return p}function c(a,b){var c,d,e,f=b.slice(0),g=a.r,h=a.b,i=g[0],j=g[1],k=g[2],l=g[3],m=g[4],n=g[5],o=g[6],p=g[7];
...
}function d(a,b){var c,d=k.random.w[a],e=[];for(c in d)d.hasOwnProperty(c)&&e.push(d[c]);
for(c=0;c<e.length;c++)e[c](b)}function e(a){window&&window.performance&&"function"==typeof window.performance.now?k.random.addEntropy(window.performance.now(),a,"loadtime"):k.random.addEntropy((new Date).valueOf(),a,"loadtime")
}function f(a){a.b=g(a).concat(g(a)),a.A=new k.cipher.aes(a.b)}function g(a){for(var b=0;4>b&&(a.f[b]=a.f[b]+1|0,!a.f[b]);b++);return a.A.encrypt(a.f)
}function h(a,b){return function(){b.apply(a,arguments)}}var i=void 0,j=!1,k={cipher:{},hash:{},keyexchange:{},mode:{},misc:{},codec:{},exception:{corrupt:function(a){this.toString=function(){return"CORRUPT: "+this.message
},this.message=a},invalid:function(a){this.toString=function(){return"INVALID: "+this.message
},this.message=a},bug:function(a){this.toString=function(){return"BUG: "+this.message
},this.message=a},notReady:function(a){this.toString=function(){return"NOT READY: "+this.message
},this.message=a}}};"undefined"!=typeof module&&module.exports&&(module.exports=k),k.cipher.aes=function(b){this.k[0][0][0]||this.D();
var c,d,e,f,g=this.k[0][4],h=this.k[1];c=b.length;var i=1;for(4!==c&&6!==c&&8!==c&&a(new k.exception.invalid("invalid aes key size")),this.b=[e=b.slice(0),f=[]],b=c;4*c+28>b;b++)d=e[b-1],(0===b%c||8===c&&4===b%c)&&(d=g[d>>>24]<<24^g[d>>16&255]<<16^g[d>>8&255]<<8^g[255&d],0===b%c&&(d=d<<8^d>>>24^i<<24,i=i<<1^283*(i>>7))),e[b]=e[b-c]^d;
for(c=0;b;c++,b--)d=e[3&c?b:b-4],f[c]=4>=b||4>c?d:h[0][g[d>>>24]]^h[1][g[d>>16&255]]^h[2][g[d>>8&255]]^h[3][g[255&d]]
},k.cipher.aes.prototype={encrypt:function(a){return b(this,a,0)},decrypt:function(a){return b(this,a,1)
},k:[[[],[],[],[],[]],[[],[],[],[],[]]],D:function(){var a,b,c,d,e,f,g,h=this.k[0],i=this.k[1],j=h[4],k=i[4],l=[],m=[];
for(a=0;256>a;a++)m[(l[a]=a<<1^283*(a>>7))^a]=a;for(b=c=0;!j[b];b^=d||1,c=m[c]||1)for(f=c^c<<1^c<<2^c<<3^c<<4,f=f>>8^255&f^99,j[b]=f,k[f]=b,e=l[a=l[d=l[b]]],g=16843009*e^65537*a^257*d^16843008*b,e=257*l[f]^16843008*f,a=0;4>a;a++)h[a][b]=e=e<<24^e>>>8,i[a][f]=g=g<<24^g>>>8;
for(a=0;5>a;a++)h[a]=h[a].slice(0),i[a]=i[a].slice(0)}},k.bitArray={bitSlice:function(a,b,c){return a=k.bitArray.P(a.slice(b/32),32-(31&b)).slice(1),c===i?a:k.bitArray.clamp(a,c-b)
},extract:function(a,b,c){var d=Math.floor(-b-c&31);return(-32&(b+c-1^b)?a[b/32|0]<<32-d^a[b/32+1|0]>>>d:a[b/32|0]>>>d)&(1<<c)-1
},concat:function(a,b){if(0===a.length||0===b.length)return a.concat(b);var c=a[a.length-1],d=k.bitArray.getPartial(c);
return 32===d?a.concat(b):k.bitArray.P(b,d,0|c,a.slice(0,a.length-1))},bitLength:function(a){var b=a.length;
return 0===b?0:32*(b-1)+k.bitArray.getPartial(a[b-1])},clamp:function(a,b){if(32*a.length<b)return a;
a=a.slice(0,Math.ceil(b/32));var c=a.length;return b&=31,c>0&&b&&(a[c-1]=k.bitArray.partial(b,a[c-1]&2147483648>>b-1,1)),a
},partial:function(a,b,c){return 32===a?b:(c?0|b:b<<32-a)+1099511627776*a},getPartial:function(a){return Math.round(a/1099511627776)||32
},equal:function(a,b){if(k.bitArray.bitLength(a)!==k.bitArray.bitLength(b))return j;
var c,d=0;for(c=0;c<a.length;c++)d|=a[c]^b[c];return 0===d},P:function(a,b,c,d){var e;
for(e=0,d===i&&(d=[]);b>=32;b-=32)d.push(c),c=0;if(0===b)return d.concat(a);for(e=0;e<a.length;e++)d.push(c|a[e]>>>b),c=a[e]<<32-b;
return e=a.length?a[a.length-1]:0,a=k.bitArray.getPartial(e),d.push(k.bitArray.partial(b+a&31,b+a>32?c:d.pop(),1)),d
},l:function(a,b){return[a[0]^b[0],a[1]^b[1],a[2]^b[2],a[3]^b[3]]}},k.codec.utf8String={fromBits:function(a){var b,c,d="",e=k.bitArray.bitLength(a);
for(b=0;e/8>b;b++)0===(3&b)&&(c=a[b/4]),d+=String.fromCharCode(c>>>24),c<<=8;return decodeURIComponent(escape(d))
},toBits:function(a){a=unescape(encodeURIComponent(a));var b,c=[],d=0;for(b=0;b<a.length;b++)d=d<<8|a.charCodeAt(b),3===(3&b)&&(c.push(d),d=0);
return 3&b&&c.push(k.bitArray.partial(8*(3&b),d)),c}},k.codec.hex={fromBits:function(a){var b,c="";
for(b=0;b<a.length;b++)c+=((0|a[b])+0xf00000000000).toString(16).substr(4);return c.substr(0,k.bitArray.bitLength(a)/4)
},toBits:function(a){var b,c,d=[];for(a=a.replace(/\s|0x/g,""),c=a.length,a+="00000000",b=0;b<a.length;b+=8)d.push(0^parseInt(a.substr(b,8),16));
return k.bitArray.clamp(d,4*c)}},k.codec.base64={J:"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/",fromBits:function(a,b,c){var d="",e=0,f=k.codec.base64.J,g=0,h=k.bitArray.bitLength(a);
for(c&&(f=f.substr(0,62)+"-_"),c=0;6*d.length<h;)d+=f.charAt((g^a[c]>>>e)>>>26),6>e?(g=a[c]<<6-e,e+=26,c++):(g<<=6,e-=6);
for(;3&d.length&&!b;)d+="=";return d},toBits:function(b,c){b=b.replace(/\s|=/g,"");
var d,e,f=[],g=0,h=k.codec.base64.J,i=0;for(c&&(h=h.substr(0,62)+"-_"),d=0;d<b.length;d++)e=h.indexOf(b.charAt(d)),0>e&&a(new k.exception.invalid("this isn't base64!")),g>26?(g-=26,f.push(i^e>>>g),i=e<<32-g):(g+=6,i^=e<<32-g);
return 56&g&&f.push(k.bitArray.partial(56&g,i,1)),f}},k.codec.base64url={fromBits:function(a){return k.codec.base64.fromBits(a,1,1)
},toBits:function(a){return k.codec.base64.toBits(a,1)}},k.hash.sha256=function(a){this.b[0]||this.D(),a?(this.r=a.r.slice(0),this.o=a.o.slice(0),this.h=a.h):this.reset()
},k.hash.sha256.hash=function(a){return(new k.hash.sha256).update(a).finalize()},k.hash.sha256.prototype={blockSize:512,reset:function(){return this.r=this.N.slice(0),this.o=[],this.h=0,this
},update:function(a){"string"==typeof a&&(a=k.codec.utf8String.toBits(a));var b,d=this.o=k.bitArray.concat(this.o,a);
for(b=this.h,a=this.h=b+k.bitArray.bitLength(a),b=512+b&-512;a>=b;b+=512)c(this,d.splice(0,16));
return this},finalize:function(){var a,b=this.o,d=this.r,b=k.bitArray.concat(b,[k.bitArray.partial(1,1)]);
for(a=b.length+2;15&a;a++)b.push(0);for(b.push(Math.floor(this.h/4294967296)),b.push(0|this.h);b.length;)c(this,b.splice(0,16));
return this.reset(),d},N:[],b:[],D:function(){function a(a){return 4294967296*(a-Math.floor(a))|0
}var b,c=0,d=2;a:for(;64>c;d++){for(b=2;d>=b*b;b++)if(0===d%b)continue a;8>c&&(this.N[c]=a(Math.pow(d,.5))),this.b[c]=a(Math.pow(d,1/3)),c++
}}},k.mode.ccm={name:"ccm",encrypt:function(b,c,d,e,f){var g,h=c.slice(0),i=k.bitArray,j=i.bitLength(d)/8,l=i.bitLength(h)/8;
for(f=f||64,e=e||[],7>j&&a(new k.exception.invalid("ccm: iv must be at least 7 bytes")),g=2;4>g&&l>>>8*g;g++);return 15-j>g&&(g=15-j),d=i.clamp(d,8*(15-g)),c=k.mode.ccm.L(b,c,d,e,f,g),h=k.mode.ccm.p(b,h,d,c,f,g),i.concat(h.data,h.tag)
},decrypt:function(b,c,d,e,f){f=f||64,e=e||[];var g=k.bitArray,h=g.bitLength(d)/8,i=g.bitLength(c),j=g.clamp(c,i-f),l=g.bitSlice(c,i-f),i=(i-f)/8;
for(7>h&&a(new k.exception.invalid("ccm: iv must be at least 7 bytes")),c=2;4>c&&i>>>8*c;c++);return 15-h>c&&(c=15-h),d=g.clamp(d,8*(15-c)),j=k.mode.ccm.p(b,j,d,l,f,c),b=k.mode.ccm.L(b,j.data,d,e,f,c),g.equal(j.tag,b)||a(new k.exception.corrupt("ccm: tag doesn't match")),j.data
},L:function(b,c,d,e,f,g){var h=[],i=k.bitArray,j=i.l;if(f/=8,(f%2||4>f||f>16)&&a(new k.exception.invalid("ccm: invalid tag length")),(4294967295<e.length||4294967295<c.length)&&a(new k.exception.bug("ccm: can't deal with 4GiB or more data")),g=[i.partial(8,(e.length?64:0)|f-2<<2|g-1)],g=i.concat(g,d),g[3]|=i.bitLength(c)/8,g=b.encrypt(g),e.length)for(d=i.bitLength(e)/8,65279>=d?h=[i.partial(16,d)]:4294967295>=d&&(h=i.concat([i.partial(16,65534)],[d])),h=i.concat(h,e),e=0;e<h.length;e+=4)g=b.encrypt(j(g,h.slice(e,e+4).concat([0,0,0])));
for(e=0;e<c.length;e+=4)g=b.encrypt(j(g,c.slice(e,e+4).concat([0,0,0])));return i.clamp(g,8*f)
},p:function(a,b,c,d,e,f){var g,h=k.bitArray;g=h.l;var i=b.length,j=h.bitLength(b);
if(c=h.concat([h.partial(8,f-1)],c).concat([0,0,0]).slice(0,4),d=h.bitSlice(g(d,a.encrypt(c)),0,e),!i)return{tag:d,data:[]};
for(g=0;i>g;g+=4)c[3]++,e=a.encrypt(c),b[g]^=e[0],b[g+1]^=e[1],b[g+2]^=e[2],b[g+3]^=e[3];
return{tag:d,data:h.clamp(b,j)}}},k.mode.ocb2={name:"ocb2",encrypt:function(b,c,d,e,f,g){128!==k.bitArray.bitLength(d)&&a(new k.exception.invalid("ocb iv must be 128 bits"));
var h,i=k.mode.ocb2.H,j=k.bitArray,l=j.l,m=[0,0,0,0];d=i(b.encrypt(d));var n,o=[];
for(e=e||[],f=f||64,h=0;h+4<c.length;h+=4)n=c.slice(h,h+4),m=l(m,n),o=o.concat(l(d,b.encrypt(l(d,n)))),d=i(d);
return n=c.slice(h),c=j.bitLength(n),h=b.encrypt(l(d,[0,0,0,c])),n=j.clamp(l(n.concat([0,0,0]),h),c),m=l(m,l(n.concat([0,0,0]),h)),m=b.encrypt(l(m,l(d,i(d)))),e.length&&(m=l(m,g?e:k.mode.ocb2.pmac(b,e))),o.concat(j.concat(n,j.clamp(m,f)))
},decrypt:function(b,c,d,e,f,g){128!==k.bitArray.bitLength(d)&&a(new k.exception.invalid("ocb iv must be 128 bits")),f=f||64;
var h,i,j=k.mode.ocb2.H,l=k.bitArray,m=l.l,n=[0,0,0,0],o=j(b.encrypt(d)),p=k.bitArray.bitLength(c)-f,q=[];
for(e=e||[],d=0;p/32>d+4;d+=4)h=m(o,b.decrypt(m(o,c.slice(d,d+4)))),n=m(n,h),q=q.concat(h),o=j(o);
return i=p-32*d,h=b.encrypt(m(o,[0,0,0,i])),h=m(h,l.clamp(c.slice(d),i).concat([0,0,0])),n=m(n,h),n=b.encrypt(m(n,m(o,j(o)))),e.length&&(n=m(n,g?e:k.mode.ocb2.pmac(b,e))),l.equal(l.clamp(n,f),l.bitSlice(c,p))||a(new k.exception.corrupt("ocb: tag doesn't match")),q.concat(l.clamp(h,i))
},pmac:function(a,b){var c,d=k.mode.ocb2.H,e=k.bitArray,f=e.l,g=[0,0,0,0],h=a.encrypt([0,0,0,0]),h=f(h,d(d(h)));
for(c=0;c+4<b.length;c+=4)h=d(h),g=f(g,a.encrypt(f(h,b.slice(c,c+4))));return c=b.slice(c),128>e.bitLength(c)&&(h=f(h,d(h)),c=e.concat(c,[-2147483648,0,0,0])),g=f(g,c),a.encrypt(f(d(f(h,d(h))),g))
},H:function(a){return[a[0]<<1^a[1]>>>31,a[1]<<1^a[2]>>>31,a[2]<<1^a[3]>>>31,a[3]<<1^135*(a[0]>>>31)]
}},k.mode.gcm={name:"gcm",encrypt:function(a,b,c,d,e){var f=b.slice(0);return b=k.bitArray,d=d||[],a=k.mode.gcm.p(!0,a,f,d,c,e||128),b.concat(a.data,a.tag)
},decrypt:function(b,c,d,e,f){var g=c.slice(0),h=k.bitArray,i=h.bitLength(g);return f=f||128,e=e||[],i>=f?(c=h.bitSlice(g,i-f),g=h.bitSlice(g,0,i-f)):(c=g,g=[]),b=k.mode.gcm.p(j,b,g,e,d,f),h.equal(b.tag,c)||a(new k.exception.corrupt("gcm: tag doesn't match")),b.data
},Z:function(a,b){var c,d,e,f,g,h=k.bitArray.l;for(e=[0,0,0,0],f=b.slice(0),c=0;128>c;c++){for((d=0!==(a[Math.floor(c/32)]&1<<31-c%32))&&(e=h(e,f)),g=0!==(1&f[3]),d=3;d>0;d--)f[d]=f[d]>>>1|(1&f[d-1])<<31;
f[0]>>>=1,g&&(f[0]^=-520093696)}return e},g:function(a,b,c){var d,e=c.length;for(b=b.slice(0),d=0;e>d;d+=4)b[0]^=4294967295&c[d],b[1]^=4294967295&c[d+1],b[2]^=4294967295&c[d+2],b[3]^=4294967295&c[d+3],b=k.mode.gcm.Z(b,a);
return b},p:function(a,b,c,d,e,f){var g,h,i,j,l,m,n,o,p=k.bitArray;for(m=c.length,n=p.bitLength(c),o=p.bitLength(d),h=p.bitLength(e),g=b.encrypt([0,0,0,0]),96===h?(e=e.slice(0),e=p.concat(e,[1])):(e=k.mode.gcm.g(g,[0,0,0,0],e),e=k.mode.gcm.g(g,e,[0,0,Math.floor(h/4294967296),4294967295&h])),h=k.mode.gcm.g(g,[0,0,0,0],d),l=e.slice(0),d=h.slice(0),a||(d=k.mode.gcm.g(g,h,c)),j=0;m>j;j+=4)l[3]++,i=b.encrypt(l),c[j]^=i[0],c[j+1]^=i[1],c[j+2]^=i[2],c[j+3]^=i[3];
return c=p.clamp(c,n),a&&(d=k.mode.gcm.g(g,h,c)),a=[Math.floor(o/4294967296),4294967295&o,Math.floor(n/4294967296),4294967295&n],d=k.mode.gcm.g(g,d,a),i=b.encrypt(e),d[0]^=i[0],d[1]^=i[1],d[2]^=i[2],d[3]^=i[3],{tag:p.bitSlice(d,0,f),data:c}
}},k.misc.hmac=function(a,b){this.M=b=b||k.hash.sha256;var c,d=[[],[]],e=b.prototype.blockSize/32;
for(this.n=[new b,new b],a.length>e&&(a=b.hash(a)),c=0;e>c;c++)d[0][c]=909522486^a[c],d[1][c]=1549556828^a[c];
this.n[0].update(d[0]),this.n[1].update(d[1]),this.G=new b(this.n[0])},k.misc.hmac.prototype.encrypt=k.misc.hmac.prototype.mac=function(b){return this.Q&&a(new k.exception.invalid("encrypt on already updated hmac called!")),this.update(b),this.digest(b)
},k.misc.hmac.prototype.reset=function(){this.G=new this.M(this.n[0]),this.Q=j},k.misc.hmac.prototype.update=function(a){this.Q=!0,this.G.update(a)
},k.misc.hmac.prototype.digest=function(){var a=this.G.finalize(),a=new this.M(this.n[1]).update(a).finalize();
return this.reset(),a},k.misc.pbkdf2=function(b,c,d,e,f){d=d||1e3,(0>e||0>d)&&a(k.exception.invalid("invalid params to pbkdf2")),"string"==typeof b&&(b=k.codec.utf8String.toBits(b)),"string"==typeof c&&(c=k.codec.utf8String.toBits(c)),f=f||k.misc.hmac,b=new f(b);
var g,h,i,j,l=[],m=k.bitArray;for(j=1;32*l.length<(e||1);j++){for(f=g=b.encrypt(m.concat(c,[j])),h=1;d>h;h++)for(g=b.encrypt(g),i=0;i<g.length;i++)f[i]^=g[i];
l=l.concat(f)}return e&&(l=m.clamp(l,e)),l},k.prng=function(a){this.c=[new k.hash.sha256],this.i=[0],this.F=0,this.s={},this.C=0,this.K={},this.O=this.d=this.j=this.W=0,this.b=[0,0,0,0,0,0,0,0],this.f=[0,0,0,0],this.A=i,this.B=a,this.q=j,this.w={progress:{},seeded:{}},this.m=this.V=0,this.t=1,this.u=2,this.S=65536,this.I=[0,48,64,96,128,192,256,384,512,768,1024],this.T=3e4,this.R=80
},k.prng.prototype={randomWords:function(b,c){var d,e=[];d=this.isReady(c);var h;
if(d===this.m&&a(new k.exception.notReady("generator isn't seeded")),d&this.u){d=!(d&this.t),h=[];
var i,j=0;for(this.O=h[0]=(new Date).valueOf()+this.T,i=0;16>i;i++)h.push(4294967296*Math.random()|0);
for(i=0;i<this.c.length&&(h=h.concat(this.c[i].finalize()),j+=this.i[i],this.i[i]=0,!(!d&&this.F&1<<i));i++);for(this.F>=1<<this.c.length&&(this.c.push(new k.hash.sha256),this.i.push(0)),this.d-=j,j>this.j&&(this.j=j),this.F++,this.b=k.hash.sha256.hash(this.b.concat(h)),this.A=new k.cipher.aes(this.b),d=0;4>d&&(this.f[d]=this.f[d]+1|0,!this.f[d]);d++);}for(d=0;b>d;d+=4)0===(d+1)%this.S&&f(this),h=g(this),e.push(h[0],h[1],h[2],h[3]);
return f(this),e.slice(0,b)},setDefaultParanoia:function(b,c){0===b&&"Setting paranoia=0 will ruin your security; use it only for testing"!==c&&a("Setting paranoia=0 will ruin your security; use it only for testing"),this.B=b
},addEntropy:function(b,c,e){e=e||"user";var f,g,h=(new Date).valueOf(),j=this.s[e],l=this.isReady(),m=0;
switch(f=this.K[e],f===i&&(f=this.K[e]=this.W++),j===i&&(j=this.s[e]=0),this.s[e]=(this.s[e]+1)%this.c.length,typeof b){case"number":c===i&&(c=1),this.c[j].update([f,this.C++,1,c,h,1,0|b]);
break;case"object":if(e=Object.prototype.toString.call(b),"[object Uint32Array]"===e){for(g=[],e=0;e<b.length;e++)g.push(b[e]);
b=g}else for("[object Array]"!==e&&(m=1),e=0;e<b.length&&!m;e++)"number"!=typeof b[e]&&(m=1);
if(!m){if(c===i)for(e=c=0;e<b.length;e++)for(g=b[e];g>0;)c++,g>>>=1;this.c[j].update([f,this.C++,2,c,h,b.length].concat(b))
}break;case"string":c===i&&(c=b.length),this.c[j].update([f,this.C++,3,c,h,b.length]),this.c[j].update(b);
break;default:m=1}m&&a(new k.exception.bug("random: addEntropy only supports number, array of numbers or string")),this.i[j]+=c,this.d+=c,l===this.m&&(this.isReady()!==this.m&&d("seeded",Math.max(this.j,this.d)),d("progress",this.getProgress()))
},isReady:function(a){return a=this.I[a!==i?a:this.B],this.j&&this.j>=a?this.i[0]>this.R&&(new Date).valueOf()>this.O?this.u|this.t:this.t:this.d>=a?this.u|this.m:this.m
},getProgress:function(a){return a=this.I[a?a:this.B],this.j>=a?1:this.d>a?1:this.d/a
},startCollectors:function(){this.q||(this.a={loadTimeCollector:h(this,this.aa),mouseCollector:h(this,this.ba),keyboardCollector:h(this,this.$),accelerometerCollector:h(this,this.U)},window.addEventListener?(window.addEventListener("load",this.a.loadTimeCollector,j),window.addEventListener("mousemove",this.a.mouseCollector,j),window.addEventListener("keypress",this.a.keyboardCollector,j),window.addEventListener("devicemotion",this.a.accelerometerCollector,j)):document.attachEvent?(document.attachEvent("onload",this.a.loadTimeCollector),document.attachEvent("onmousemove",this.a.mouseCollector),document.attachEvent("keypress",this.a.keyboardCollector)):a(new k.exception.bug("can't attach event")),this.q=!0)
},stopCollectors:function(){this.q&&(window.removeEventListener?(window.removeEventListener("load",this.a.loadTimeCollector,j),window.removeEventListener("mousemove",this.a.mouseCollector,j),window.removeEventListener("keypress",this.a.keyboardCollector,j),window.removeEventListener("devicemotion",this.a.accelerometerCollector,j)):document.detachEvent&&(document.detachEvent("onload",this.a.loadTimeCollector),document.detachEvent("onmousemove",this.a.mouseCollector),document.detachEvent("keypress",this.a.keyboardCollector)),this.q=j)
},addEventListener:function(a,b){this.w[a][this.V++]=b},removeEventListener:function(a,b){var c,d,e=this.w[a],f=[];
for(d in e)e.hasOwnProperty(d)&&e[d]===b&&f.push(d);for(c=0;c<f.length;c++)d=f[c],delete e[d]
},$:function(){e(1)},ba:function(a){k.random.addEntropy([a.x||a.clientX||a.offsetX||0,a.y||a.clientY||a.offsetY||0],2,"mouse"),e(0)
},aa:function(){e(2)},U:function(a){a=a.accelerationIncludingGravity.x||a.accelerationIncludingGravity.y||a.accelerationIncludingGravity.z;
var b="";window.orientation&&(b=window.orientation),k.random.addEntropy([a,b],3,"accelerometer"),e(0)
}},k.random=new k.prng(6);a:try{var l,m,n;if("undefined"!=typeof module&&module.exports)m=require("crypto"),l=m.randomBytes(128),k.random.addEntropy(l,1024,"crypto['randomBytes']");
else if(window&&Uint32Array){if(n=new Uint32Array(32),window.crypto&&window.crypto.getRandomValues)window.crypto.getRandomValues(n);
else{if(!window.msCrypto||!window.msCrypto.getRandomValues)break a;window.msCrypto.getRandomValues(n)
}k.random.addEntropy(n,1024,"crypto['getRandomValues']")}}catch(o){console.log("There was an error collecting entropy from the browser:"),console.log(o)
}k.json={defaults:{v:1,iter:1e3,ks:128,ts:64,mode:"ccm",adata:"",cipher:"aes"},Y:function(b,c,d,e){d=d||{},e=e||{};
var f,g=k.json,h=g.e({iv:k.random.randomWords(4,0)},g.defaults);return g.e(h,d),d=h.adata,"string"==typeof h.salt&&(h.salt=k.codec.base64.toBits(h.salt)),"string"==typeof h.iv&&(h.iv=k.codec.base64.toBits(h.iv)),(!k.mode[h.mode]||!k.cipher[h.cipher]||"string"==typeof b&&100>=h.iter||64!==h.ts&&96!==h.ts&&128!==h.ts||128!==h.ks&&192!==h.ks&&256!==h.ks||2>h.iv.length||4<h.iv.length)&&a(new k.exception.invalid("json encrypt: invalid parameters")),"string"==typeof b?(f=k.misc.cachedPbkdf2(b,h),b=f.key.slice(0,h.ks/32),h.salt=f.salt):k.ecc&&b instanceof k.ecc.elGamal.publicKey&&(f=b.kem(),h.kemtag=f.tag,b=f.key.slice(0,h.ks/32)),"string"==typeof c&&(c=k.codec.utf8String.toBits(c)),"string"==typeof d&&(d=k.codec.utf8String.toBits(d)),f=new k.cipher[h.cipher](b),g.e(e,h),e.key=b,h.ct=k.mode[h.mode].encrypt(f,c,h.iv,d,h.ts),h
},encrypt:function(){var a=k.json,b=a.Y.apply(a,arguments);return a.encode(b)},X:function(b,c,d,e){d=d||{},e=e||{};
var f=k.json;c=f.e(f.e(f.e({},f.defaults),c),d,!0);var g;return d=c.adata,"string"==typeof c.salt&&(c.salt=k.codec.base64.toBits(c.salt)),"string"==typeof c.iv&&(c.iv=k.codec.base64.toBits(c.iv)),(!k.mode[c.mode]||!k.cipher[c.cipher]||"string"==typeof b&&100>=c.iter||64!==c.ts&&96!==c.ts&&128!==c.ts||128!==c.ks&&192!==c.ks&&256!==c.ks||!c.iv||2>c.iv.length||4<c.iv.length)&&a(new k.exception.invalid("json decrypt: invalid parameters")),"string"==typeof b?(g=k.misc.cachedPbkdf2(b,c),b=g.key.slice(0,c.ks/32),c.salt=g.salt):k.ecc&&b instanceof k.ecc.elGamal.secretKey&&(b=b.unkem(k.codec.base64.toBits(c.kemtag)).slice(0,c.ks/32)),"string"==typeof d&&(d=k.codec.utf8String.toBits(d)),g=new k.cipher[c.cipher](b),d=k.mode[c.mode].decrypt(g,c.ct,c.iv,d,c.ts),f.e(e,c),e.key=b,k.codec.utf8String.fromBits(d)
},decrypt:function(a,b,c,d){var e=k.json;return e.X(a,e.decode(b),c,d)},encode:function(b){var c,d="{",e="";
for(c in b)if(b.hasOwnProperty(c))switch(c.match(/^[a-z0-9]+$/i)||a(new k.exception.invalid("json encode: invalid property name")),d+=e+'"'+c+'":',e=",",typeof b[c]){case"number":case"boolean":d+=b[c];
break;case"string":d+='"'+escape(b[c])+'"';break;case"object":d+='"'+k.codec.base64.fromBits(b[c],0)+'"';
break;default:a(new k.exception.bug("json encode: unsupported type"))}return d+"}"
},decode:function(b){b=b.replace(/\s/g,""),b.match(/^\{.*\}$/)||a(new k.exception.invalid("json decode: this isn't json!")),b=b.replace(/^\{|\}$/g,"").split(/,/);
var c,d,e={};for(c=0;c<b.length;c++)(d=b[c].match(/^(?:(["']?)([a-z][a-z0-9]*)\1):(?:(\d+)|"([a-z0-9+\/%*_.@=\-]*)")$/i))||a(new k.exception.invalid("json decode: this isn't json!")),e[d[2]]=d[3]?parseInt(d[3],10):d[2].match(/^(ct|salt|iv)$/)?k.codec.base64.toBits(d[4]):unescape(d[4]);
return e},e:function(b,c,d){if(b===i&&(b={}),c===i)return b;for(var e in c)c.hasOwnProperty(e)&&(d&&b[e]!==i&&b[e]!==c[e]&&a(new k.exception.invalid("required parameter overridden")),b[e]=c[e]);
return b},ea:function(a,b){var c,d={};for(c in a)a.hasOwnProperty(c)&&a[c]!==b[c]&&(d[c]=a[c]);
return d},da:function(a,b){var c,d={};for(c=0;c<b.length;c++)a[b[c]]!==i&&(d[b[c]]=a[b[c]]);
return d}},k.encrypt=k.json.encrypt,k.decrypt=k.json.decrypt,k.misc.ca={},k.misc.cachedPbkdf2=function(a,b){var c,d=k.misc.ca;
return b=b||{},c=b.iter||1e3,d=d[a]=d[a]||{},c=d[c]=d[c]||{firstSalt:b.salt&&b.salt.length?b.salt.slice(0):k.random.randomWords(2,0)},d=b.salt===i?c.firstSalt:b.salt,c[d]=c[d]||k.misc.pbkdf2(a,d,b.iter),{key:c[d].slice(0),salt:d.slice(0)}
},window.sjcl=k}(),function(a){"use strict";var b=a.jQuery,c=function(a){function c(){if(!e.isPaused&&null===d&&0!==e.queue.length){var a,f=e.queue.shift();
...
}),function(a){"use strict";var b;b=a.BMP,b.waitForBlinkGap=function(){var b,c,d,e;
return b=new $.Deferred,c=new Date,e=function(){document.removeEventListener("deviceready",e,!1),b.resolve()
},d=function(){a.isBlinkGap||a.PhoneGap||a.cordova?a.PhoneGap&&a.PhoneGap.available?b.resolve():document.addEventListener&&document.addEventListener("deviceready",e,!1):$.now()-c>1e4?b.reject(new Error("waitForBlinkGap(): still no PhoneGap after 10 seconds")):setTimeout(d,197)
},d(),b.promise()}}(this),function(a,b){"use strict";"function"==typeof define&&define.amd?define(["../../../../."],function(c){return a.BMP=b(a,c),a.BMP
...
return c.hideLocationBar=function(){a.scrollTo(0,1)},c.prepareView=function(c){var d=new b.Deferred,e=b(".view:visible").not(c[0]),f=b("#navBoxHeader");
return MyAnswers.dispatch.add(function(){c.hide(),c.css({"z-index":0,position:"absolute"}),currentMasterCategory?c.attr({"data-name":siteVars.config["m"+currentMasterCategory].pertinent.name}):b("#categoriesView").removeAttr("data-name"),currentCategory?c.attr({"data-name":siteVars.config["c"+currentCategory].pertinent.name}):b("#keywordListView").removeAttr("data-name"),currentInteraction?c.attr({"data-name":siteVars.config["i"+currentInteraction].pertinent.name,"data-type":siteVars.config["i"+currentInteraction].pertinent.type}):(b("#answerView").removeAttr("data-name"),b("#answerView").removeAttr("data-type")),e.css({"z-index":50,position:"absolute"}),c.show(),"screen-bottom"!==a.currentConfig.footerPosition&&MyAnswers.$body.children("footer").hide(),f.find("button").prop("disabled",!0),d.resolve()
}),d.promise()},c.showView=function(d,e){var f=new b.Deferred,g=b(".view:visible").not(d[0]),h=e?"right":"left";
return c.hideLocationBar(),MyAnswers.dispatch.add(function(){0!==g.length?g.hide("slide",{direction:h},300,function(){g.css("z-index",""),g.css("position",""),d.css("z-index",""),d.css("position",""),a.updateNavigationButtons(),MyAnswers.$body.children("footer").show(),f.resolve(),d.trigger("viewShow")
}):(d.css("z-index",""),d.css("position",""),a.updateNavigationButtons(),MyAnswers.$body.children("footer").show(),f.resolve(),d.trigger("viewShow"))
}),f.promise()},c};a.MyAnswersDevice=new c}(this),document.getElementById("startUp-loadDevice").className="working success",MyAnswers.deviceDeferred.resolve(),function(a){"use strict";
...
}"BlinkSideBar"===g.attr("id")&&(h="list",i=currentConfig.sidebarDisplay),log("MyAnswers.populateItemListing(): "+a+" "+h+" "+i+" "+n+"["+k.join(",")+"]");
try{switch(h){case"list":u=1;break;case"2 column":u=2;break;case"3 column":u=3;break;
case"4 column":u=4}for(t=j.length,s=0;t>s;s++)w=siteVars.config[n+j[s]],w&&-1!==$.inArray(j[s],k)&&"hide"!==w.pertinent.display?(o=w.pertinent.displayName||w.pertinent.name,"text only"!==i&&w.pertinent.icon?(p=$("<img />"),p.attr({"class":"v"+u+"col",src:w.pertinent.icon,alt:o}),l.append(p)):(p=$("<li />"),q=$('<div class="label" />'),q.text(o),p.append(q),"string"==typeof w.pertinent.description&&(r=$('<div class="description" />'),r.text(w.pertinent.description),p.append(r)),m.append(p)),p.attr("data-id",j[s]),x[a](p)):error(n+j[s]+" not in siteVars.config")
}catch(b){error("MyAnswers.populateItemListing: ",b)}}),f.dispatch.add(function(){try{l.children().length>0&&(v=l.find("img"),1===u?(v.first().addClass("topLeft topRight"),v.last().addClass("bottomLeft bottomRight")):(v.first().addClass("topLeft"),v.length>=u&&v.eq(u-1).addClass("topRight"),v.length%u===0&&(v.eq(-1*u).addClass("bottomLeft"),v.last().addClass("bottomRight"))),l.appendTo(g)),m.children().length>0&&m.appendTo(g)
}catch(a){error("MyAnswers.populateItemListing: ",a)}})}}(this),function(a){var b,c,d,e,f=a.jQuery,g=a.MyAnswers,h=a.deviceVars,i={},j=f("#BlinkSideBar"),k=f("#stackLayout");
...
},function(a){function b(){var b,c;return c=v("#startUp-initBlinkGap"),c.addClass("working"),a.isBlinkGap||a.PhoneGap||a.cordova?(log("waitForBlinkGap(): native application detected"),b=a.BMP.waitForBlinkGap()):(log("waitForBlinkGap(): native application not detected"),b=new v.Deferred,b.resolve()),b.then(function(){c.addClass("success")
},function(b){c.addClass("error"),(a.isBlinkGap||a.PhoneGap||a.cordova)&&warn(b.message)
}),b.promise()}function c(){var b=new v.Deferred,c=null,d=function(a){c=null,x.notification.alert("Database installed, please restart the App.",function(){x.gap_database.requestTerminate()
...
s.config["a"+s.id].pertinent}log("navigating to requested screen (if any)..."),delete s.queryParameters.keyword,b&&!v.isEmptyObject(s.queryParameters)?(a="/"+s.answerSpace+"/"+s.config["i"+b].pertinent.name+"/?"+v.param(s.queryParameters),o.pushState({i:b,arguments:s.queryParameters},null,a)):b?(a="/"+s.answerSpace+"/"+s.config["i"+b].pertinent.name+"/?",o.pushState({i:b},null,a)):"string"==typeof s.queryParameters._c?(a="/"+s.answerSpace+"/?_c="+s.queryParameters._c,o.pushState({c:s.queryParameters._c},null,a)):"string"==typeof s.queryParameters._m&&(a="/"+s.answerSpace+"/?_m="+s.queryParameters._m,o.pushState({m:s.queryParameters._m},null,a)),delete s.queryParameters
}()}b.remove(),v("#content").show(),setSubmitCachedFormButton(),u.device.persistentStorage&&(u.dfrdMoJOs=processMoJOs())
}function l(){var b=u.device.persistentStorage;log("loaded():");try{b&&u.store.set("answerSpace",s.answerSpace),v.when(b?u.siteStore.get("config"):!0).then(function(a){"string"==typeof a&&(a=v.parseJSON(a)),"object"===v.type(a)&&(info("loading stored siteVars.config",JSON.stringify(a)),s.config=a)
}).always(function(){v.when(b?u.siteStore.get("map"):!0).then(function(a){"string"==typeof a&&(a=v.parseJSON(a)),"object"===v.type(a)&&(info("loading stored siteVars.map"),s.map=a)
}).always(function(){v.when(requestLoginStatus()).always(function(){v.when(requestConfig()).always(function(){s.map&&s.config?(k(),Modernizr.orientation&&y.on("orientationchange",f),y.on("resize",f),f()):(w.append("error: unable to contact server, insufficient data found in local storage"),a.cordova&&cordova.offline&&w.append("status: found cordova.offline"))
})})})}),v.when(b?u.store.get("starsProfile"):!0).then(function(a){"string"===v.type(a)&&(a=v.parseJSON(a)),"object"===v.type(a)&&(starsProfile=a)
}),v("#startUp-initLoaded").addClass("success")}catch(c){log("loaded(): exception:"),error(c),v("#startUp-initLoaded").addClass("error"),w.append("loading error: "+c)
...
}catch(d){error("Error in onStateChange handler..."),error(d)}return a.preventDefault(),!1
}),-1!==location.href.indexOf("index.php?answerSpace=")&&o.replaceState(null,null,"/"+s.answerSpace+"/"),null!==r.getElementById("loginButton")&&(s.hasLogin=!0),v.when(b()).then(function(){isBlinkGapDevice()&&"function"===v.type(onDeviceReady)&&onDeviceReady()
...
}).then(function(){v("#startUp-loadPolyFills").addClass("success"),v("#startUp-initBrowser").addClass("working"),setTimeout(n,isBlinkGapDevice()?2e3:193)
})})})}(this),document.getElementById("startUp-loadMain").className="working success",MyAnswers.mainDeferred.resolve();

//# sourceMappingURL=android.min.map