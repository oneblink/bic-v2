...
if(200===b.status||0===b.status)try{c=$.parseJSON(b.responseText)}catch(d){warn(d)
}processConfig(c),200===b.status||304===b.status||0===b.status?a.resolve():a.reject()
}}):window.cordova&&window.cordova.offline?cordova.offline.retrieveContent(function(b){try{b=$.parseJSON(b)
}catch(c){warn(c)}processConfig(b),a.resolve()},function(b){warn("requestConfig.Retrieve: failed "),processConfig(),a.reject(b)
},{url:b}):(processConfig(),a.reject()),a.promise()}function gotoStorageView(){var a,b;
if(currentConfig.activeFormsUseInteraction){if(a=siteVars.config["i"+currentConfig.activeFormsInteraction],"object"!==$.type(a))return void BMP.alert("Interaction for active forms listing could not be found.");
...
}function l(){var b=u.device.persistentStorage;log("loaded():");try{b&&u.store.set("answerSpace",s.answerSpace),v.when(b?u.siteStore.get("config"):!0).then(function(a){"string"==typeof a&&(a=v.parseJSON(a)),"object"===v.type(a)&&(s.config=a)
}).always(function(){v.when(b?u.siteStore.get("map"):!0).then(function(a){"string"==typeof a&&(a=v.parseJSON(a)),"object"===v.type(a)&&(s.map=a)
