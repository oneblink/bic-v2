...
dashcode.setupParts = function() {
	...
	if (!DC.Support.BorderImage) {
		...
		for ( var B in G) {
			var D = G[B];
			Element.set3PiecesBorderImage(document.getElementById(B), D[0],
					D[1], D[2])
		}
	}
	for ( var C in dashcodePartSpecs) {
...
dashcode.setupPart = function(elementOrId, creationFunction, viewClass,
		specDict, relativeController) {
	var object = null;
	...
	if (createFunc) {
		...
		if (!viewClass) {
			viewClass = DC.View.viewClassForNode(node, specDict.hasBindings)
					|| DC.View
		}
		object = new (viewClass)(node, relativeController,
...
dashcode.setupDataSource = function(identifier, specDict) {
	...
	if (!dsClass) {
		console.error("Couldn't create data source " + identifier
...
dashcode.processClonedTemplateElement = function(D, C, B, F, E, A) {
	console
			.error("dashcode.processClonedTemplateElement is no longer available.")
};
...
if (!("querySelector" in document)) {
	document
			.write('<script apple-no-regeneration="yes" type="text/javascript" src="../Parts/core/external/sizzle_c.js"><\/script>')
};
...
DC.Support = {
	...
	BorderImage :( function() {
		...
		return (A.WebkitBorderImage == "inherit")
				|| (A.MozBorderImage == "inherit")
	})()
};
...
if ("undefined" !== typeof (window.Prototype)) {
	( function() {
		var A = [ "indexOf", "lastIndexOf", "forEach", "filter", "map", "some",
...
var Class = ( function() {
	...
	return {
		create : function(L, J) {
			...
			case 0:
				throw new TypeError(
						"Missing superclass and declaration arguments");
			case 1:
				J = L;
				L = undefined;
				break;
			...
			if ("function" == typeof (J)) {
				...
				if (!J) {
					throw new Error(
							"Class declaration function did not return a prototype")
				}
			}
...
DC.ChangeNotification = Class.create( {
	...
	toString : function() {
		...
		A += " newValue=" + this.newValue + " oldValue=" + this.oldValue
...
DC.ObserverEntry = Class.create( {
	...
	observeChangeForKeyPath : function(A, B) {
		if (!this.callback || !this.observer
				|| -1 !== A.objectKeyPath.indexOf(this.observer)) {
			return
		}
...
DC.KVO = Class
		.create( {
			...
			setKeysTriggerChangeNotificationsForDependentKey : function(D, C) {
				...
				if (!C) {
					throw new InvalidArgumentError(
							"dependentKey can not be null")
				}
				if (-1 !== C.indexOf(".")) {
					throw new InvalidArgumentError(
							"dependentKey may not be a key path")
				}
				var B;
				var F;
				var A;
				...
				for (A = 0; A < D.length; ++A) {
					B = D[A];
					if (!B) {
						throw new InvalidArgumentError("key at index " + A
								+ " was null")
					}
			...
			_addParentLink : function(D, E, C) {
				...
				var A = new DC.ObserverEntry(D,
						D.observeChildObjectChangeForKeyPath, E ? E.key : "");
				B[C] = A;
				if (!E) {
					return
				}
			...
			didChangeValueForKey : function(B, F) {
				...
				if (C !== A) {
					var E = new DC.ChangeNotification(this,
							DC.ChangeType.setting, C, A);
			...
			notifyObserversOfChangeForKeyPath : function(J, L) {
				...
				for (M in this.__observers) {
					...
					P = new DC.ChangeNotification(J.object, J.changeType, C, O,
							J.indexes);
					H = F.length;
					for (G = 0; G < H; ++G) {
...
DC.KVO.keysToIgnore = $S("__keys", "__observers", "__keysToIgnore",
		"__dependentKeys", "__mutableKeys", "__factories__");
...
DC.KVO.getPropertyMethodsForKeyOnObject = ( function() {
	...
	function E(T, K) {
		...
		var S;
		var F = K[I];
		var N = ("undefined" !== typeof (O = K.__lookupGetter__(T)) && "undefined" !== typeof (H = K
				.__lookupSetter__(T)));
		if (!N) {
			P = (P in K) ? P : T;
			O = K[P];
...
DC.Bindable = Class.create(DC.KVO, {
	...
	__createObserverMethod : function(B, C) {
		function A(E) {
			if (DC.ChangeType.setting !== E.changeType) {
				throw new InvalidArgumentError(
						'Received invalid change type for synthesized binding observer (name="'
								+ B + '" keyPath="' + C + '")')
			}
			var D = E.newValue;
	...
	bindNameToKeyPath : function(B, G, A) {
		...
		D = this["observe" + B.titleCase() + "Change"]
				|| this.__createObserverMethod(B, G);
		if (this.bindings[B]) {
		...
		.create( {
			constructor : function(D, A, B) {
				...
				if ("string" != C && "function" != C) {
					throw new InvalidArgumentError(
							"comparisonFn must be either the name of a method or a function reference")
				}
			},
			resolveComparisonFn : function(B) {
				...
				if ("function" !== typeof (A)) {
					throw new TypeError(
							"comparisonFn does not resolve to a function")
				}
				return A
			},
			compareObjects : function(C, B) {
				if (!C.valueForKeyPath || !B.valueForKeyPath) {
					throw new InvalidArgumentError(
							"Objects are not Key Value compliant")
				}
...
DC.findTransformerWithName = function(transformerName) {
	var valueTransformer = DC.transformerInstances[transformerName
			.toLowerCase()];
	if (valueTransformer) {
	...
	if (!valueTransformer) {
		throw new InvalidArgumentError(
				"The transformerName argument does not specify a valid ValueTransformer: "
						+ transformerName)
	}
...
DC.registerTransformerWithName = function(B, A) {
	if (!B.transformedValue) {
		throw new InvalidArgumentError(
				"The valueTransformer argument does not support the ValueTransformer method transformedValue")
	}
	A = A.toLowerCase();
...
DC.registerTransformerWithName(new DC.transformer.StringsToObjects("string"),
		"StringsToObjects");
		...
		.create( {
			constructor : function(A) {
				...
				if ("string" === typeof (this.transformer)) {
					this.transformer = DC
							.findTransformerWithName(this.transformer)
				}
			...
			bind : function() {
				this.object.addObserverForKeyPath(this,
						this.observeChangeForKeyPath, this.keypath)
			},
			unbind : function() {
			...
			validateProposedValue : function(B) {
				if (this.transformer) {
					if (!this.transformer.reverseTransformedValue) {
						throw new Error(
								"Can't validate a value when the transformer doesn't have a reverseTransformedValue method")
					}
			...
			mutable : function() {
				if (this.transformer
						&& !this.transformer.reverseTransformedValue) {
					return false
				}
			...
			update : function() {
				var B = this.value();
				var C = new DC.ChangeNotification(this.object,
						DC.ChangeType.setting, B);
				this.updating = true;
				try {
					this.observerFn.call(this.observer, C, this.keypath)
				} catch (A) {
					console.error('Exception while bindng "' + this.name
							+ '" to keypath "' + this.keypath + ' ": ' + A)
				}
				this.updating = false
			},
			...
			placeholderValue : function() {
				...
				if ((this.markerType + "Placeholder") in this) {
					...
					A = this.observer.defaultPlaceholderForMarkerWithBinding(
							this.markerType, this.name)
				}
				if ("function" === typeof (A)) {
		...
		.extend(
				Array,
				{
					...
					addObject : function(B) {
						var A = this.length;
						var C = new DC.ChangeNotification(this,
								DC.ChangeType.insertion, [ B ], null, [ A ]);
						this.push(B);
						this.observeElementAtIndex(A);
						this.notifyObserversOfChangeForKeyPath(C,
								DC.KVO.kAllPropertiesKey)
					},
					addObjects : function(G) {
						...
						var C = [];
						var A = [];
						var F = new DC.ChangeNotification(this,
								DC.ChangeType.insertion, C, null, A);
						...
						this.notifyObserversOfChangeForKeyPath(F,
								DC.KVO.kAllPropertiesKey)
					},
					...
					insertObjectsAtIndexes : function(E, C) {
						if (E.length !== C.length) {
							throw new InvalidArgumentError(
									"length of objects and indexes parameters must be equal")
						}
						var A = E.length;
						var D;
						var B;
						...
						var F = new DC.ChangeNotification(this,
								DC.ChangeType.insertion, E, null, C);
						this.notifyObserversOfChangeForKeyPath(F,
								DC.KVO.kAllPropertiesKey)
					},
					replaceObjectAtIndex : function(C, B) {
						var A = this[B];
						this[B] = C;
						var D = new DC.ChangeNotification(this,
								DC.ChangeType.replacement, [ C ], [ A ], [ B ]);
						this.notifyObserversOfChangeForKeyPath(D,
								DC.KVO.kAllPropertiesKey)
					},
					replaceObjectsAtIndexes : function(F, D) {
						...
						var G = new DC.ChangeNotification(this,
								DC.ChangeType.replacement, F, null, D);
						this.notifyObserversOfChangeForKeyPath(G,
								DC.KVO.kAllPropertiesKey)
					},
					...
					removeObjectsAtIndexes : function(D) {
						...
						var G = new DC.ChangeNotification(this,
								DC.ChangeType.deletion, null, B, E);
						this.notifyObserversOfChangeForKeyPath(G,
								DC.KVO.kAllPropertiesKey)
					},
					removeObjectAtIndex : function(B) {
						...
						var C = new DC.ChangeNotification(this,
								DC.ChangeType.deletion, null, A, [ B ]);
						this.notifyObserversOfChangeForKeyPath(C,
								DC.KVO.kAllPropertiesKey)
					},
					removeAllObjects : function() {
						...
						var E = new DC.ChangeNotification(this,
								DC.ChangeType.deletion, null, B, C);
						this.notifyObserversOfChangeForKeyPath(E,
								DC.KVO.kAllPropertiesKey)
					},
					...
					observeChildObjectChangeForKeyPath : function(F, E, B) {
						...
						var A = new DC.ChangeNotification(D,
								DC.ChangeType.replacement, [ F.newValue ],
								[ F.oldValue ], [ C ]);
...
DC.ObjectController = Class.create(DC.Controller,
		{
			...
			observeChildObjectChangeForKeyPath : function(F, E, B) {
				...
				var A = new DC.ChangeNotification(this, DC.ChangeType.setting,
						D, null);
...
function IndexRange(C, A) {
	var B;
	var D = [];
	if (1 == arguments.length && C.length) {
		A = C.length - 1;
		C = 0
	}
	for (B = C; B <= A; ++B) {
...
DC.ArrayController = Class.create(DC.ObjectController, {
	...
	exposedBindings : [ "selectionIndexes", "sortDescriptors",
			"filterPredicate", "contentForMultipleSelection" ],
	observeContentChange : function(D) {
		var A;
		var C;
		switch (D.changeType) {
		case DC.ChangeType.setting:
			this.setContent(D.newValue);
			break;
		case DC.ChangeType.insertion:
			this._insertObjectsIntoArrangedObjects(D.newValue);
			if (this.selectsInsertedObjects && !this.selectedObjects()) {
				var B = this.selectedObjects();
				B = B.concat(D.newValue);
				this.setSelectedObjects(B)
			} else {
				if (!this.selectionIndexes().length
						&& this.avoidsEmptySelection()) {
	...
	rearrangeObjects : function(D) {
		...
		if (C.length && this.avoidsEmptySelection() && !E.length) {
			E.push(0)
		}
	...
	setAvoidsEmptySelection : function(A) {
		this.__avoidsEmptySelection = A;
...
Object.extend(Element, {
	...
	set3PiecesBorderImage : function(G, B, E, J) {
		if (DC.Support.BorderImage) {
			G.style.borderWidth = B ? "0px " + J + "px 0px " + E + "px "
					: "0px";
	...
	getRect : function(P, N) {
		...
		var E = K ? [ "display", "overflow", "borderLeftWidth",
				"borderTopWidth" ] : [ "display" ];
	...
	onDomReady : function(A) {
		...
		if (!Event._readyCallbacks) {
			document.addEventListener("DOMContentLoaded",
					Event._domHasFinishedLoading, false);
			function B() {
...
DC.View = Class.create(DC.Responder, {
	exposedBindings : [ "visible", "class", "enabled", "editable", "html",
			"text" ],
	defaultPlaceholders : {
		text : {
...
DC.ImageView = Class.create(DC.View, {
	...
	init : function() {
		this.base();
		var A = this.viewElement();
		if (!this.defaultPlaceholders.src && A.src) {
			var B = {};
			B.nullValue = B.noSelection = A.src;
			this.defaultPlaceholders.src = B
		}
		Event.observe(A, "error", this.onerror.bind(this));
		Event.observe(A, "load", this.onload.bind(this))
	},
	onload : function() {
		var A = this.viewElement();
		this.setValueForKey(false, "loading");
		Element.removeClassName(A, DC.Style.kLoadingClass)
	},
	onerror : function() {
		var A = this.viewElement();
		this.setValueForKey(false, "loading");
		Element.updateClass(A, DC.Style.kInvalidValueClass,
				DC.Style.kLoadingClass)
	},
	src : function() {
		var A = this.viewElement().src;
		return ("about:blank" === A) ? null : A
	},
	setSrc : function(C) {
		if (this.bindings.src) {
			this.bindings.src.setValue(C)
		}
		this.setValueForKey(true, "loading");
		var B = this.viewElement();
		Element.updateClass(B, DC.Style.kLoadingClass,
		...
		var A = B.src;
		B.src = C;
		if (DC.Browser.Safari && B.src === A) {
			this.onload()
		}
	},
	observeSrcChange : function(C) {
		var B = this.viewElement();
		var A = this.bindings.src && this.bindings.src.markerType;
		if (A) {
			Element.addClassName(B, DC.Style.kMarkerClass)
		} else {
			Element.removeClassName(B, DC.Style.kMarkerClass)
		}
		this.setSrc(C.newValue)
	},
	observeWidthChange : function(C) {
		var A = this.viewElement();
		var B = parseInt(C.newValue, 10);
		if (isNaN(B)) {
...
DC.DashcodePart = Class.create(DC.View, {
	...
	_sendLegacyOnClick : function(D, C, E) {
		...
		if (document.createEvent) {
			...
			A.initMouseEvent("click", false, false, window, 1, D ? D.screenX
					: 0, D ? D.screenY : 0, D ? D.clientX : 0, D ? D.clientY
					: 0, D ? D.ctrlKey : false, D ? D.altKey : false,
					D ? D.shiftKey : false, D ? D.metaKey : false, D ? D.button
							: 0, null);
			C.dispatchEvent(A)
		} else {
			C.onclick(D)
	...
	observeEnabledChange : function(A) {
		if (this.__initialising
				&& (null === A.newValue || "undefined" === A.newValue)) {
	...
	Replacement : {
		View : {
			createViewForNode : function(B, C, A) {
				return DC.DashcodeOverrides.Original.View.createViewForNode(B,
						C, A)
			},
			viewClassForNode : function(node, hasBindings) {
				var viewClass = undefined;
				if (node && (node.originalID || node.id)) {
					var spec = dashcodePartSpecs[node.originalID ? node.originalID
							: node.id];
					if (spec && spec.view) {
						try {
				...
				if (!viewClass) {
					viewClass = DC.DashcodeOverrides.Original.View
							.viewClassForNode(node, hasBindings)
				}
				return viewClass
			}
		},
		Binding : {
			bindingFromString : function(B, A) {
				if (DC.DashcodeOverrides.bindToContent
						&& (A instanceof DC.ObjectController)) {
					if ((B.substr(0, 7) != "content")) {
						B = "content." + B
					}
				}
				return DC.DashcodeOverrides.Original.Binding.bindingFromString(
						B, A)
			}
		}
	}
...
DC.FormControl = Class.create(DC.View, {
	...
	maskedBindings : [ "text", "html" ],
	observeValueChange : function(D) {
		var B = this.viewElement();
		var C = D.newValue;
		var A = this.bindings.value && this.bindings.value.markerType;
		if (DC.NoSelectionMarkerType === A) {
			B.disabled = true
		} else {
		...
		if (!this.bindings.editable) {
			B.readOnly = !this.bindings.value.mutable()
		}
		if (B.readOnly) {
			Element.addClassName(B, DC.Style.kReadOnlyClass)
		} else {
			Element.removeClassName(B, DC.Style.kReadOnlyClass)
		}
		if (B.disabled) {
			Element.addClassName(B, DC.Style.kDisabledClass)
		} else {
			Element.removeClassName(B, DC.Style.kDisabledClass)
		}
		B.value = C
	},
	observeNameChange : function(D, C, B) {
		var A = this.viewElement();
		A.name = D.newValue
	},
	validate : function() {
...
DC.TextField = Class.create(DC.FormControl,
		{
			init : function() {
				this.base();
				var A = this.viewElement();
				Event.observe(A, "change", this.valueChanged
						.bindAsEventListener(this));
				Event.observe(A, "drop", this.fieldReceivedDropEvent
						.bindAsEventListener(this));
				this.editing = false;
			...
			validate : function() {
				...
				if (this.formatter) {
					...
					C = this.formatter.valueForString(C)
				}
				if (this.bindings.value) {
					C = this.bindings.value.validateProposedValue(C);
					if (C instanceof DC.Error) {
						this.presentError(C);
						return C
					}
			...
			presentError : function(A) {
				...
				Element.addClassName(this.viewElement(),
						DC.Style.kInvalidValueClass);
				return this.base.apply(this, arguments)
			},
			clearAllErrors : function() {
				this.validationError = null;
				Element.removeClassName(this.viewElement(),
						DC.Style.kInvalidValueClass);
				return this.base.apply(this, arguments)
			},
			acceptsFirstResponder : function() {
			...
			showPlaceholder : function() {
				var A = this.viewElement();
				if (this.bindings.value) {
					A.value = this.bindings.value.placeholderValue()
				} else {
					if (this.placeholder) {
				...
				Element.addClassName(A, DC.Style.kMarkerClass)
			},
			hidePlaceholder : function() {
				...
				Element.removeClassName(A, DC.Style.kMarkerClass)
			},
			valueChanged : function(A) {
				...
				var B = this.validate();
				if (!(B instanceof DC.Error) && this.bindings.value) {
			...
			fieldReceivedDropEvent : function(B) {
				var A = this.viewElement();
				A.value = ""
			},
			onkeyup : function(B) {
				...
				this.updateTimer = this.valueChanged.bindAndDelay(this,
						this.keypressUpdateTimeout)
			},
			onkeydown : function(A) {
				if (this.action && A.keyCode === Event.KEY_RETURN) {
			...
			onkeypress : function(A) {
				...
				if (!A.charCode || A.altKey || A.metaKey || A.ctrlKey) {
					return
				}
				var B = String.fromCharCode(A.charCode || A.keyCode);
				if (!this.formatter.isValidInputCharacter(B)) {
					Event.stop(A)
				}
			},
			observeValueChange : function(D) {
				...
				var A = this.bindings.value && this.bindings.value.markerType;
				if (!A && this.formatter) {
					C = this.formatter.stringForValue(C)
				}
				if (DC.NoSelectionMarkerType === A) {
					B.disabled = true
				} else {
				...
				DC.DashcodePart,
				{
					...
					init : function() {
						...
						case "SELECT":
							...
							Event.observe(B, "change",
									this.selectedIndexChanged
											.bindAsEventListener(this));
							break;
						case "TABLE":
							A = this.setContainer(B.tBodies[0]);
						...
						default:
							...
							this._items = PartList(C.nodeName);
							this.templateElement = C;
							DC.View.createViewsForNodeTree(C, null,
									this.__context);
						...
						if ("true" === (B.getAttribute("multiple") || "")
								.toLowerCase()) {
							this.multiple = true
						}
						if (this.template
						...
						if (this.template
								&& !(this.template.valueOf()).__factoryFn__) {
					...
					clonedFrom : function(A) {
						if (A.templateElement) {
							this.templateElement = A.templateElement;
					...
					highlightSelection : function(B, A) {
						return
					},
					highlightSelectionIndexes : function(E) {
						var K = this.viewElement();
						var J = this._items();
						var G = J.length;
						var F;
						var C = !!this._isSelectElement;
						var D;
						var B = DC.Style.kSelectedClass;
						var I;
						var M;
						if (this.animated) {
							var N = {
								curve :DC.easing.inOutSine,
								duration :300
							};
							I = function(P, O) {
								DC.Animator.addClassName(P, O, N)
							};
							M = function(P, O) {
								DC.Animator.removeClassName(P, O, N)
							}
						} else {
							I = Element.addClassName;
							M = Element.removeClassName
						}
						if (!C) {
							var H = this.__selectionIndexes ? this.__selectionIndexes.length
									: 0;
							for (F = 0; F < H; F++) {
								var A = this.__selectionIndexes[F];
								if (E.containsObject(A)) {
									continue
								}
								if (A < J.length) {
									J[A].selected = false;
									M(J[A], B);
									this.highlightSelection(J[A], false)
								}
							}
							for (F = 0; F < E.length; F++) {
								...
								J[L].selected = true;
								I(J[L], B);
								this.highlightSelection(J[L], true)
							}
						}
						if (1 === E.length) {
							this.selectedIndex = K.selectedIndex = E[0]
						}
						this.__selectionIndexes = E
					},
					selectionIndexes : function() {
						return this.__selectionIndexes
					},
					setSelectionIndexes : function(E) {
						var A;
						var B = this.viewElement();
						var F = this.bindings;
						E = (E || []).concat();
						if (String(E) === String(this.__selectionIndexes)) {
							E = this.__selectionIndexes
						}
						if (1 === E.length) {
							A = this.selectedIndex = B.selectedIndex = E[0]
						} else {
							if (E.length) {
								A = B.selectedIndex = E[0]
							} else {
								A = B.selectedIndex = -1
							}
						}
						if (F.selectionIndexes) {
							F.selectionIndexes.setValue(E)
						}
						if (F.selectedIndex) {
							F.selectedIndex.setValue(A)
						}
						if (F.selectedObject && F.content) {
							var C = null;
							if (-1 !== A) {
								C = (this.__content || [])[A]
							}
							F.selectedObject.setValue(C)
						}
						if (F.selectedValue) {
							var D = null;
							if (-1 !== A) {
							...
							F.selectedValue.setValue(D)
						}
						this.highlightSelectionIndexes(E)
					},
					selectedObjects : function() {
						var A = this.__content || [];
						return A
								.objectsAtIndexes(this.selectionIndexes() || [])
					},
					onkeydown : function(B) {
						var J = this.viewElement();
						if (J.disabled) {
							return
						}
						if (Event.KEY_UP != B.keyCode
								&& Event.KEY_DOWN != B.keyCode) {
							return
						}
						Event.stop(B);
						var I = this.selectionIndexes().sort( function(P, O) {
							return P - O
						});
						var E = (this.__content || []).length - 1;
						var L;
						if (this.multiple && B.shiftKey && I.length) {
							var H = (this.selectedIndex == I[0]);
							var G = I[0];
							var K = I[I.length - 1];
							if (1 == I.length) {
								if (Event.KEY_UP == B.keyCode) {
						...
						if (!isNaN(L)) {
							var N = this._items(L);
							var M = Element.scrollParent(J);
							var F = M.offsetHeight;
							var C = N.offsetTop - M.scrollTop;
						...
						this.setSelectionIndexes(I);
						return
					},
					selectedIndexChanged : function(C) {
						if (!this.bindings.selectionIndexes
								&& !this.bindings.selectedIndex
								&& !this.bindings.selectedObject
								&& !this.bindings.selectedValue) {
							return
						}
						var A = this.viewElement();
						var B;
						if (this.multiple) {
							...
							B = [ this.selectedIndex = A.selectedIndex ]
						}
						this.setSelectionIndexes(B)
					},
					_clearSelection : function() {
						var A = this.viewElement();
						var E = ("SELECT" !== A.tagName);
						var F = Element.removeClassName;
						var C = DC.Style.kSelectedClass;
						function D(G) {
							G.selected = false;
							if (E) {
								F(G, C)
					...
					observeSelectedIndexChange : function(D) {
						var C = D.newValue;
						var B = (-1 !== C && null !== C && "undefined" !== typeof (C));
						var A = B ? [ C ] : [];
						this.setSelectionIndexes(A)
					},
					...
					_updateDomWithContent : function(I) {
						var K = DC.View.rebindNodeTreeWithRelativeSource;
						var O = DC.View.unbindNodeTree;
						var P = this.selectedObjects();
						var G = [];
						var C = I.length;
						var J = this._items();
						var L = J.length;
						...
						while (F < D) {
							...
							if ("undefined" === typeof (A)) {
								this.setupElement(H, H.objectValue)
							} else {
								K(H, A, E)
							}
							M = DC.View.fromNode(H);
							if (!M || !M.bindings.visible) {
								H.style.display = ""
							}
							this.elementUsedForIndex(H, F);
							++F
						}
						if (F < C) {
							...
							while (F < C) {
								...
								H = this.createElement(E, null, N, F);
								this._items.add(H);
								++F
							}
							B.appendChild(N)
							...
							while (F < L) {
								H = J[F];
								if (F > this.maximumCachedRows) {
									this.removeChild(J[F])
								} else {
									O(H);
									H.objectValue = null;
									H.style.display = "none";
									this.elementUsedForIndex(H, -1)
								}
								++F
							}
							if (L > this.maximumCachedRows) {
								this._items.refresh()
							}
						}
						this.highlightSelectionIndexes(G)
					},
					observeContentChange : function(L) {
						var A = this.container();
						var N = this.viewElement();
						var Q = this;
						var H;
						var M;
						var I;
						...
						var G;
						var F = [];
						var O = DC.View.rebindNodeTreeWithRelativeSource;
						var P = DC.View.unbindNodeTree;
						var D = K[0];
						var J = this.__content ? this.__content.length - 1 : 0;
						function C(U, T, S) {
							var R = U.objectValue;
							U.objectValue = T;
							O(U, R, T);
							U.style.display = "";
							Q.elementUsedForIndex(U, S)
						}
						switch (L.changeType) {
						case DC.ChangeType.setting:
							this.__content = L.newValue || [];
							this._updateDomWithContent(this.__content);
							break;
						case DC.ChangeType.insertion:
							J -= L.indexes.length;
							for (H = 0; H < L.indexes.length; ++H) {
								M = this._items(L.indexes[H]);
								if (M && "none" === M.style.display) {
									I = M;
									C(M, L.newValue[H], L.indexes[H])
								} else {
									I = this.createElement(L.newValue[H], M,
											null, L.indexes[H]);
									this._items.insertPartAtIndex(I,
											L.indexes[H])
								}
								if (this.animated) {
									Element.addClassName(I,
											DC.Style.kInsertedClass);
									DC.Animator
											.removeClassName(
													I,
													DC.Style.kInsertedClass,
						...
						case DC.ChangeType.replacement:
							function E(R) {
								C(R, R.newObjectValue);
								R.newObjectValue = null;
								DC.Animator.removeClassName(R,
										DC.Style.kReplacingClass, {
											duration :250,
											discreteTransitionPoint :1
										})
							}
							for (H = 0; H < L.indexes.length; ++H) {
								I = this._items(L.indexes[H]);
								if (!this.animated) {
									C(I, L.newValue[H], L.indexes[H])
								} else {
									I.newObjectValue = L.newValue[H];
									DC.Animator
											.addClassName(
													I,
													DC.Style.kReplacingClass,
						...
						case DC.ChangeType.deletion:
							J += L.indexes.length;
							F = this.selectionIndexes();
							for (H = L.indexes.length - 1; H >= 0; --H) {
								var B = L.indexes[H];
								F.removeObject(B);
								I = this._items(B);
								this._items.removePartAtIndex(B);
								if (!this.animated) {
									this.removeChild(I)
								} else {
									DC.Animator
											.addClassName(
													I,
													DC.Style.kDeletedClass,
													{
														duration :this.updateAnimationDuration,
														discreteTransitionPoint :0,
														callback :this.removeChild
																.bind(this)
													})
								}
							}
							...
							break;
						default:
							console.log("Unknown change type: " + L.changeType);
							break
						}
						if (this.__content && this.__content.length) {
							if (K[0] != D) {
								Element
										.addClassName(K[0],
												DC.Style.kFirstClass);
								Element
										.removeClassName(D,
												DC.Style.kFirstClass)
							}
							if (J != (this.__content.length - 1)) {
								Element.addClassName(
										K[this.__content.length - 1],
										DC.Style.kLastClass);
								if (J < K.length) {
									Element.removeClassName(K[J],
											DC.Style.kLastClass)
								}
							}
						} else {
					...
					setupElement : function(D, C) {
						D.objectValue = C;
						if (this.template) {
							var A = DC.dataModel;
							var B;
							DC.dataModel = this.__context;
							this.template.call(this, D, C);
							DC.dataModel = A
						} else {
							DC.View
									.createViewsForNodeTree(D, C,
											this.__context)
						}
						return D
					},
					createElement : function(B, H, C, F) {
						if (!this.templateElement) {
							console.log("No template row");
							return
						}
						var I = this.viewElement();
						var G = null;
						var E = null;
						C = C || this.container();
						if (this.template) {
							E = Element.clone(this.templateElement);
							if (H) {
								G = C.insertBefore(E, H)
							} else {
								G = C.appendChild(E)
							}
							G = this.setupElement(G, B)
						} else {
							E = DC.View.cloneViewsForTreeNode(
									this.templateElement, B, this.__context);
							if (!H) {
								var A = C.children;
								var D = null;
								if (A && A.length) {
						...
						this.elementUsedForIndex(G, F);
						return G
					},
					elementUsedForIndex : function(B, A) {
						return
					},
					...
					onmousedown : function(D) {
						...
						while (E && E.parentNode != A) {
							E = E.parentNode
						}
						if (E == A) {
							return
						}
						this._activeItem = C.indexOf(E);
					...
					onmouseup : function(B) {
						...
						if (-1 !== this._activeItem) {
							Element.removeClassName(this
									._items(this._activeItem),
									DC.Style.kActiveClass)
						}
						this._activeItem = -1
					},
					...
					onclick : function(A) {
						...
						var F;
						if (!this.multiple) {
							if ((A.ctrlKey || A.metaKey) && this.allowsEmpty) {
								F = this.selectionIndexes();
								if (-1 != F.indexOf(D)) {
									D = -1
								}
							...
							F = this.selectionIndexes().slice();
							if (A.shiftKey) {
								var J = [];
								if (this.selectedIndex > D) {
									J = IndexRange(D, this.selectedIndex)
								} else {
									J = IndexRange(this.selectedIndex, D)
								}
								this.setSelectionIndexes(J)
							} else {
								if (A.ctrlKey || A.metaKey) {
									...
									if (-1 === E) {
										...
										if (this.allowsEmpty || (F.length > 1)) {
											F.splice(E, 1)
										}
									}
									this.setSelectionIndexes(F)
...
DC.SelectField = Class.create(DC.ListView, {
	exposedBindings : [ "displayValues", "contentValues", "selectedValue",
			"name" ],
	observeContentValuesChange : function(G) {
		var B = this.viewElement();
		var E;
		var D;
		var F = B.options;
		...
		case DC.ChangeType.setting:
			...
			for (E = 0; E < A; ++E) {
				F[E].value = G.newValue[E]
			}
			break;
		case DC.ChangeType.insertion:
		case DC.ChangeType.replacement:
			var C;
			for (C = 0; C < G.indexes.length; ++C) {
				E = G.indexes[C];
				D = F[E];
				D.value = G.newValue[C]
			}
			break;
		default:
	...
	observeDisplayValuesChange : function(G) {
		var B = this.viewElement();
		var E;
		var D;
		var F = B.options;
		...
		case DC.ChangeType.setting:
			...
			for (E = 0; E < A; ++E) {
				if (DC.Browser.IE) {
					F[E].innerText = G.newValue[E]
				} else {
					F[E].text = G.newValue[E]
				}
			}
			break;
		case DC.ChangeType.insertion:
	...
	observeSelectedValueChange : function(F, H, B) {
		if (this.bindings.content && !this.bindings.content.value()) {
			return
		}
		var G = this.viewElement();
		var I = G.options;
		var E = I.length;
		var A = F.newValue;
		G.disabled = "undefined" === typeof (A)
				|| DC.Markers.MultipleValues === A
				|| DC.Markers.NoSelection === A;
		var D = [];
		for ( var C = 0; C < E; ++C) {
			if (I[C].value == A) {
				D = [ C ];
...
DC.Page = Class.create(DC.Responder,
		{
			...
			constructor : function() {
				...
				this._onmousedragHandler = this._onmousedrag
						.bindAsEventListener(this);
				this._delegates = {
					click : []
				};
				...
				if ("string" === typeof (C)) {
					...
					for (D in C) {
						if (!(D in this._delegates)) {
							throw new Error("Invalid delegation event type: "
									+ D)
						}
			...
			_ontouchstart : function(C) {
				...
				if (A) {
					var B = this;
					A.ontouchstart(C);
					this._touchstartMouseDownDelay = window.setTimeout(
							function() {
								A.onmousedown(C);
								B._touchsentMD = true;
...
( function() {
	...
	window.setTimeout = function(F, E) {
		...
		if ("string" === typeof (F)) {
			F = "DC.EventLoop.begin();do {" + F
					+ "} while (false); DC.EventLoop.end();";
	...
	if (DC.Browser.IE) {
		 ...
		 * This file was generated by Dashcode and is covered by the license.txt
		 * included in the project. You may edit this file, however it is
		 * recommended to first turn off the Dashcode code generator otherwise
...
var dashcodePartSpecs = {
	...
	"list" : {
		...
		"dataSourceName" :"keywordController",
		"labelElementId" :"keywordLabel",
		"listStyle" :"List.EDGE_TO_EDGE",
		"sampleRows" :5,
...
/*
 * This file was generated by Dashcode and is covered by the license.txt
 * included in the project. You may edit this file, however it is recommended to
 * first turn off the Dashcode code generator otherwise the changes will be
 * lost.
 */
...
Transition.prototype.perform = function(G, B, E) {
	if (!G || !G.parentNode) {
		return
	}
	var F = G.parentNode;
	if (B) {
		...
		A.top = B.offsetTop + "px";
		if (this._useTransforms) {
			A.webkitTransitionProperty = "none";
			A.webkitTransform = this._translateOp(0, 0);
			A.webkitBackfaceVisibility = ""
		}
		if (!this.type || this.type == Transition.NONE_TYPE
				|| !this._useTransforms) {
			if ((this.type != Transition.FADE_TYPE)
					&& (this.type != Transition.SLIDE_TYPE) || E) {
				A.display = "none"
			}
		}
	}
	F.style.overflow = "hidden";
	var D = Element.getStyles(F, "position");
	if ((D != "absolute") && (D != "relative")) {
		F.style.position = "relative"
	}
	var C = G.style;
	if (this._useTransforms) {
		C.webkitTransitionProperty = "none";
		C.webkitTransform = this._translateOp(0, 0);
		C.webkitBackfaceVisibility = "";
	...
	if (this.startTransitionCallback) {
		this.startTransitionCallback(this, B, G)
	}
	if (this.type && this.type != Transition.NONE_TYPE && this._useTransforms) {
		...
		this._newView = G;
		this._oldView = B;
		this._previousNewStyleOpacity = C.opacity;
		this._shouldHideOldView = true;
		if (B) {
			A.position = "absolute";
			this._preventEventsInContainer();
			this._originalContainerElementHeight = F.style.height;
			F.style.height = Math.max(B.offsetHeight, G.offsetHeight) + "px"
		}
		if (this._pushContainer && (this.type != Transition.FLIP_TYPE)) {
			this._clearPushContainer()
		}
		if (this.type == Transition.DISSOLVE_TYPE
				|| this.type == Transition.FADE_TYPE) {
			this._performFadeTransition(E)
		} else {
			if (this.type == Transition.PUSH_TYPE
					|| this.type == Transition.SLIDE_TYPE) {
		...
		if (this._privateEndTransitionCallback) {
			this._privateEndTransitionCallback(this, B, G)
		}
		if (this.endTransitionCallback) {
			this.endTransitionCallback(this, B, G)
		}
	}
};
Transition.areTransformsSupported = function() {
	if (!Transition._areTransformsSupported) {
		Transition._areTransformsSupported = (window.WebKitCSSMatrix ? true
				: false)
	}
...
Transition._findAnimationRule = function(E) {
	...
	for ( var C = 0; C < F.length; C++) {
		var H = F[C];
		if (D.test(H.href)) {
			for ( var B = 0; B < H.cssRules.length; B++) {
				var G = H.cssRules[B];
...
Transition.prototype._animationEnded = function(A) {
	if (!this._checkedForEnded) {
		...
		if (this.type == Transition.FLIP_TYPE) {
			Transition._removeClassName(this._containerElement,
					"dashcode-transition-flip-container");
			Transition._removeClassName(this._pushContainer,
...
Transition.prototype._performFadeTransition = function(E) {
	if (this._oldView) {
		...
		Transition._addDelayedTransitionCallback( function() {
			...
			if (C || (F && !E)) {
				A._setupTransition(D, "opacity", G, A.timing, "opacity",
						A._previousNewStyleOpacity)
			}
		});
...
Transition.prototype._performPushOrSlideTransition = function(A) {
	if (this._oldView) {
		...
		var J = true;
		var H = A ? -1 : 1;
		var D = parseInt(this._containerWidth, 10);
		...
		if (this.direction == Transition.LEFT_TO_RIGHT_DIRECTION
				|| this.direction == Transition.TOP_TO_BOTTOM_DIRECTION) {
			H *= -1
		}
		if (B || (F && !A)) {
		...
		Transition._addDelayedTransitionCallback( function() {
			var L = I._getDurationString();
			if (B || (F && A)) {
				var K;
				if (J) {
					K = I._translateOp(-1 * H * D, 0)
				} else {
					K = I._translateOp(0, -1 * H * D)
				}
				I._setupTransition(C, "-webkit-transform", L, I.timing,
						"webkitTransform", K)
			} else {
				I._shouldHideOldView = false
			}
			if (B || (F && !A)) {
				I._setupTransition(E, "-webkit-transform", L, I.timing,
						"webkitTransform", I._translateOp(0, 0))
			}
		});
		Event.observe(this._newView, "webkitTransitionEnd", this)
	}
...
Transition.prototype._performFlipTransition = function(B) {
	if (this._oldView) {
		var H = this._newView.style;
		var E = this._oldView.style;
		var G = parseInt(this._containerWidth, 10);
		if (G != 320) {
			if (Transition._containerFlipTranslateZStyle === undefined) {
				var C = Transition
						._findAnimationRule("dashcode-transition-flip-container-pushback");
				try {
					Transition._containerFlipTranslateZStyle = C
							.findRule("50%").style
				} catch (J) {
		...
		if (Transition._containerFlipTranslateZStyle) {
			Transition._containerFlipTranslateZStyle.webkitTransform = "translateZ("
					+ -1 * G / 2 + "px)"
		}
		if (!this._pushContainer) {
			this._pushContainer = document.createElement("div");
			this._pushContainer.setAttribute(
					"apple-transition-flip-push-container", "true");
			this._pushContainer
					.setAttribute(
							"style",
							"position: relative; top: 0; left: 0; overflow: visible; z-index: 0; -webkit-transform-style: preserve-3d;");
			var D = this._containerElement.childNodes;
			for ( var I = D.length - 1; I >= 0; I--) {
				if (D[I] != this._mask) {
					this._pushContainer.appendChild(D[I])
				}
			}
			this._containerElement.appendChild(this._pushContainer)
		}
		var M = this._getDurationString();
		var K = this.direction;
		if (K != Transition.RIGHT_TO_LEFT_DIRECTION
				&& K != Transition.LEFT_TO_RIGHT_DIRECTION) {
			K = Transition.RIGHT_TO_LEFT_DIRECTION
		}
		var A = ((K == Transition.RIGHT_TO_LEFT_DIRECTION) && !B)
				|| ((K == Transition.LEFT_TO_RIGHT_DIRECTION) && B);
		E.webkitAnimationDuration = M;
		E.webkitAnimationTimingFunction = this.timing;
		var L = A ? "dashcode-transition-flip-right-old-view"
				: "dashcode-transition-flip-left-old-view";
		Transition._addClassName(this._oldView, L);
		H.webkitAnimationDuration = M;
		H.webkitAnimationTimingFunction = this.timing;
		var F = A ? "dashcode-transition-flip-right-new-view"
				: "dashcode-transition-flip-left-new-view";
		Transition._addClassName(this._newView, F);
		this._pushContainer.style.webkitAnimationDuration = M;
		this._pushContainer.style.webkitAnimationTimingFunction = this.timing;
		Transition._addClassName(this._pushContainer,
				"dashcode-transition-flip-container-pushback");
		this._containerElement.style.webkitAnimationDuration = M;
		this._containerElement.style.webkitAnimationTimingFunction = this.timing;
		Transition._addClassName(this._containerElement,
				"dashcode-transition-flip-container");
		Event.observe(this._containerElement, "webkitAnimationEnd", this);
		this._newViewAnimationName = F;
		this._oldViewAnimationName = L
	}
};
Transition.prototype._performCubeTransition = function(A) {
	if (this._oldView) {
		...
		var E = this._newView.style;
		var C = this._oldView.style;
		var I = this._getDurationString();
		var H = this.direction;
		if (H != Transition.RIGHT_TO_LEFT_DIRECTION
				&& H != Transition.LEFT_TO_RIGHT_DIRECTION) {
			H = Transition.RIGHT_TO_LEFT_DIRECTION
		}
		var B = ((H == Transition.RIGHT_TO_LEFT_DIRECTION) && !A)
				|| ((H == Transition.LEFT_TO_RIGHT_DIRECTION) && A);
		...
		var D = parseInt(this._containerWidth, 10);
		E.webkitTransitionProperty = "none";
		E.webkitTransform = G._rotateOp("Y", F * 90) + " translateZ(" + D
				+ "px)";
		Transition._addDelayedTransitionCallback( function() {
			var J = G._getDurationString();
			G._setupTransition(C, "-webkit-transform", J, G.timing,
					"webkitTransform", G._rotateOp("Y", F * -90)
							+ " translateZ(" + D + "px)");
			G._setupTransition(E, "-webkit-transform", J, G.timing,
...
Transition.prototype._performSwapTransition = function(D) {
	if (this._oldView) {
		...
		if (F != Transition.RIGHT_TO_LEFT_DIRECTION
				&& F != Transition.LEFT_TO_RIGHT_DIRECTION) {
			F = Transition.RIGHT_TO_LEFT_DIRECTION
		}
		var B = ((F == Transition.RIGHT_TO_LEFT_DIRECTION) && !D)
				|| ((F == Transition.LEFT_TO_RIGHT_DIRECTION) && D);
		A.webkitAnimationDuration = H;
		A.webkitAnimationTimingFunction = this.timing;
		var E = B ? "dashcode-transition-swap-right-old-view"
		...
		var G = B ? "dashcode-transition-swap-right-new-view"
...
Transition.prototype._performRevolveTransition = function(C) {
	if (this._oldView) {
		...
		if (D != Transition.RIGHT_TO_LEFT_DIRECTION
				&& D != Transition.LEFT_TO_RIGHT_DIRECTION) {
			D = Transition.RIGHT_TO_LEFT_DIRECTION
		}
		if (D == Transition.RIGHT_TO_LEFT_DIRECTION) {
			E = C ? "dashcode-transition-revolve-right-old-view"
					: "dashcode-transition-revolve-right-reverse-old-view"
		} else {
			E = C ? "dashcode-transition-revolve-left-old-view"
		...
		var F;
		if (D == Transition.RIGHT_TO_LEFT_DIRECTION) {
			F = C ? "dashcode-transition-revolve-right-new-view"
					: "dashcode-transition-revolve-right-reverse-new-view"
		} else {
			F = C ? "dashcode-transition-revolve-left-new-view"
...
Transition._addDelayedTransitionCallback = function(C) {
	var B = Transition;
	if (!B._delayedCallbacks) {
		B._delayedCallbacks = new Array();
		var A = function() {
			var D = B._delayedCallbacks.length;
			for ( var E = 0; E < D; E++) {
				B._delayedCallbacks[E]()
			}
			delete B._delayedCallbacks
		};
		setTimeout(A, 0)
	}
	B._delayedCallbacks.push(C)
};
DC.StackLayout = Class
		.create(
				DC.DashcodePart,
				{
					...
					clonedFrom : function(A) {
						this.base(A);
						this._viewsOldOpacity = A._viewsOldOpacity
					},
					partSetup : function(I) {
						this.base(I);
						var D = I.subviewsTransitions || [];
						var B = this.viewElement();
						var E = I.originalID ? I.originalID : B.originalID;
						this._viewsOldOpacity = [];
						this._views = [];
						this._currentView = null;
						var A = B.children;
						this._inDesign = window.dashcode
								&& window.dashcode.inDesign;
						var H = false;
						for ( var C = 0; C < A.length; C++) {
							var G = A[C];
							this._views.push(G);
							if (!this._inDesign && !E) {
								G.style.display = (H) ? "none" : "block";
								if (this._viewsOldOpacity.length <= C) {
									this._viewsOldOpacity.push(G.style.opacity)
								}
								G.style.opacity = (H) ? 0 : 1;
								H = true
							}
						}
						if (this._views.length > 0) {
							...
							if (this._views.length == D.length) {
								for ( var C = 0; C < this._views.length; C++) {
									this._viewsTransition[C] = CreateTransitionWithProperties(D[C])
								}
							}
						}
						...
						var F = this.viewElement();
						do {
							this._topPosFromBody += F.offsetTop
						} while (F = F.offsetParent);
						this._maskContainerElement = B
					},
					...
					setCurrentView : function(A) {
						this.setCurrentViewWrapped(A, arguments[1],
								arguments[2])
					},
					setCurrentViewWrapped : function(E, C, B) {
						E = this._getView(E);
						var A = this.getCurrentView();
						if (!E || (A == E)) {
							return
						}
						if (!E.parentNode == this.viewElement()) {
							return
						}
					...
					setCurrentViewWithTransition : function(E, D, C, B) {
						E = this._getView(E);
						if (!E) {
							return
						}
					...
					getTransitionForView : function(A) {
						A = this._getView(A);
					...
					addView : function(A, B) {
						...
						A.style.display = (this._views.length == 1 ? "none"
								: "block");
					...
					removeView : function(B) {
						...
						this.viewElement().removeChild(B)
					},
					_indexOfView : function(A) {
						var B = -1;
					...
					_getView : function(A) {
						if (A) {
							...
							for ( var B = 0; B < this._views.length; B++) {
								var C = this._views[B];
								if ((C.id == A) || (C.originalID == A)) {
									return C
								}
							}
						}
					...
					_setRestrictToBrowserTransition : function(A) {
					...
					_setCurrentViewPrimitive : function(H, D, G, E, C) {
						if (!this._inDesign) {
							if (this.startTransitionCallback) {
								this.startTransitionCallback(this, D, H)
							}
							if (C) {
								var F = this._topPosFromBody
										- window.pageYOffset;
								if (F < 0) {
									window.scrollBy(0, F)
								}
							...
							if (G) {
								this._restoreOldOpacity(D);
								this._restoreOldOpacity(H);
								var B = this._getRealTransition(G);
								var A = this;
								B._privateEndTransitionCallback = function(I) {
									A._transitionEnded(I, D, H)
								};
								B._maskContainerElement = this._maskContainerElement;
								B.perform(H, D, E)
							} else {
								if (D) {
								...
								this._transitionEnded(null, D, H)
							}
						}
						this._currentView = H
					},
					_transitionEnded : function(B, A, C) {
						if (this._privateEndTransitionCallback) {
							this._privateEndTransitionCallback(this, A, C)
						}
						if (this.endTransitionCallback) {
							this.endTransitionCallback(this, A, C)
						}
					},
					_restoreOldOpacity : function(A) {
						if (A) {
		...
		.create(
				DC.DashcodePart,
				{
					...
					partSetup : function(spec) {
						...
						var _self = this;
						var originalID = spec.originalID ? spec.originalID
						...
						if (originalID) {
							while (buttonElement.firstChild) {
								buttonElement
										.removeChild(buttonElement.firstChild)
							}
						}
						...
						var imagePrefix = "Parts/Images/"
								+ (originalID ? originalID : buttonElement.id);
						...
						this._backgroundElementPressed = this._backgroundElement
								.cloneNode(false);
						style = this._backgroundElementPressed.style;
						style.visibility = "hidden";
						buttonElement
								.appendChild(this._backgroundElementPressed);
						this._containerElement = this._backgroundElement
								.cloneNode(false);
						...
						this._containerElement
								.appendChild(this._lineBreakElement);
						this._eventsElement = this._backgroundElement
								.cloneNode(false);
						...
						var text = spec.text || "";
						if (window.dashcode && dashcode.getLocalizedString) {
							text = dashcode.getLocalizedString(text)
						}
						this.setText(text);
						this.setImageURL(spec.customImage || "");
						this.setPressedImageURL(spec.customImagePressed || "");
						var imagePosition = (spec.customImagePosition == undefined)
								|| (spec.customImagePosition.length < 1) ? DC.PushButton.IMAGE_POSITION_NONE
								: eval(spec.customImagePosition);
						this.setImagePosition(imagePosition);
						if (spec.onclick && !dashcode.inDesign) {
							try {
								var onClickHandler = eval(spec.onclick);
								if (onClickHandler) {
									this._setOnClickAsAction(onClickHandler,
											this._eventsElement)
								}
							} catch (e) {
							}
					...
					setEnabled : function(A) {
						...
						var D = 1;
						var B = this.viewElement();
						var C = Element.getStyles(B, [ "opacity" ]);
						if (C && C.opacity != null) {
							D = +C.opacity
						}
						if (A) {
							B.style.opacity = D
									* (1 / DC.PushButton.DISABLED_OPACITY);
							B.style.appleDashboardRegion = "dashboard-region(control rectangle)"
						} else {
							B.style.opacity = DC.PushButton.DISABLED_OPACITY
									* D;
							B.style.appleDashboardRegion = "none"
						}
					},
					getEnabled : function() {
						return this.enabled
					...
					observeEnabledChange : function(A) {
						...
						this.setEnabled(A.newValue)
					},
					observeTextChange : function(C, B, A) {
						if (this.__initialising
								&& (null === C.newValue || "undefined" === typeof (C.newValue))) {
							this.bindings.text.setValue(this.getText());
							return
						}
						this.setText(C.newValue)
					},
					setText : function(A) {
						this._textElement.innerText = A;
						if (this._sizeToFit) {
							this.sizeToFit()
						}
					},
					...
					sizeToFit : function(A, B) {
						var C = this._widthRequiredToFit();
						if (B && (B < C)) {
							C = parseInt(B, 10)
						} else {
							if (A && (A > C)) {
						...
						this.viewElement().style.width = C + "px";
						this._layoutElements()
					},
					setImagePosition : function(A) {
						this._imagePosition = A;
						this._layoutElements()
					},
					...
					setPressedImageURL : function(B) {
						this._customImagePressedURL = B;
						...
						if (this._customImagePressedURL) {
							...
							this._customImagePressed.onload = function() {
								A._customImagePressedLoaded = true;
								A._setCustomImage()
							};
					...
					setState : function(A) {
						this._pressed = (A == DC.PushButton.STATE_ON);
						if (this._pressed) {
					...
					getState : function() {
						return this._pressed ? DC.PushButton.STATE_ON
								: DC.PushButton.STATE_OFF
					},
					_setCustomImage : function() {
						var A = this._imageElement.style;
						if (this._pressed && this._customImagePressed
								&& this._customImagePressedLoaded) {
							this._imageElement.src = this._customImagePressedURL;
							A.width = this._customImagePressed.width + "px";
							A.height = this._customImagePressed.height + "px"
						} else {
							if (!this._pressed && this._customImage
									&& this._customImageLoaded) {
								this._imageElement.src = this._customImageURL;
								A.width = this._customImage.width + "px";
								A.height = this._customImage.height + "px"
							} else {
					...
					_setPressed : function(B) {
						var A = B ? DC.PushButton.STATE_ON
								: DC.PushButton.STATE_OFF;
						this.setState(A)
					},
					_updateBackgroundImage : function() {
						var B = this._backgroundElement.style;
						var C = this._backgroundElementPressed.style;
						if (this._noBackground) {
							Element.set3PiecesBorderImage(
									this._backgroundElement, null);
							Element.set3PiecesBorderImage(
									this._backgroundElementPressed, null);
							B.backgroundImage = "";
							C.backgroundImage = ""
						} else {
							if (this._leftImageWidth > 0
									|| this._rightImageWidth > 0) {
								Element.set3PiecesBorderImage(
								...
								Element.set3PiecesBorderImage(
										this._backgroundElementPressed,
										this._pressedBgImageURL,
										this._leftImageWidth,
										this._rightImageWidth);
								B.backgroundImage = "";
								C.backgroundImage = ""
								...
								Element.set3PiecesBorderImage(
										this._backgroundElementPressed, null);
								B.backgroundImage = "url(" + this._bgIimageURL
										+ ")";
						...
						var A = 2;
						this._containerLeft = +this._leftImageWidth + A;
						this._containerRight = +this._rightImageWidth + A;
						if (this._containerLeft > this._containerRight) {
							this._containerLeft = Math
									.ceil((this._containerRight + this._containerLeft) / 2)
						} else {
							if (this._containerRight > this._containerLeft) {
								this._containerRight = Math
										.ceil((this._containerRight + this._containerLeft) / 2)
							}
						}
						this._containerElement.style.marginLeft = this._containerLeft
								+ "px";
						this._containerElement.style.marginRight = this._containerRight
								+ "px"
					},
					_layoutElements : function() {
						var R = this._imagePosition;
						if (!this._customImage
								|| (!this._customImageLoaded
										&& !this._customImagePressedLoaded && (R != DC.PushButton.IMAGE_POSITION_CENTER))) {
							R = DC.PushButton.IMAGE_POSITION_NONE
						}
						var J = this._imageElement.style;
						var E = this._textElement.style;
						var K = this._containerElement.style;
						var P = this._lineBreakElement.style;
						var G = this.viewElement().offsetHeight;
						if (G < 1) {
							G = this._initialHeight
						}
						var L = this.viewElement.offsetWidth;
						if (L < 1) {
							L = this._initialWidth
						}
						if (R == DC.PushButton.IMAGE_POSITION_NONE) {
							E.position = "static";
							J.display = "none";
							E.display = "inline";
							K.lineHeight = G + "px";
							...
							var A = this._textElement.offsetHeight;
							if (A < 1) {
								var O = Element.getStyles(this._textElement,
										[ "font-size" ]);
								if (O && O["font-size"] != null) {
									A = parseFloat(O["font-size"])
								}
							}
							var H = 0;
							var Q = 0;
							...
							var N = 5;
							if (this._pressed && this._customImagePressed) {
								H = this._customImagePressed.width;
								Q = this._customImagePressed.height
							} else {
								if (this._customImage) {
									H = this._customImage.width;
									Q = this._customImage.height
								}
							}
							if (R == DC.PushButton.IMAGE_POSITION_LEFT
									|| R == DC.PushButton.IMAGE_POSITION_RIGHT) {
								E.display = "inline";
								J.display = "inline";
								J.marginTop = "";
								J.marginBottom = "";
								...
								P.display = "none";
								if (R == DC.PushButton.IMAGE_POSITION_LEFT) {
									this._containerElement.insertBefore(
											this._imageElement,
											this._textElement);
									J.marginLeft = "";
									J.marginRight = B + "px"
								...
								if (R == DC.PushButton.IMAGE_POSITION_TOP
										|| R == DC.PushButton.IMAGE_POSITION_BOTTOM) {
									E.display = "inline";
									J.display = "inline";
									J.marginLeft = "";
									J.marginRight = "";
									K.lineHeight = "";
									K.overflow = "hidden";
									var F = Math.floor((G - (A + Q + N)) / 2);
									K.marginTop = Math.max(F, 0) + "px";
									P.display = "inline";
									if (R == DC.PushButton.IMAGE_POSITION_TOP) {
										J.marginTop = "";
										J.marginBottom = N + "px";
									...
									P.display = "none";
									var C = H
											- (L - this._containerLeft - this._containerRight);
									var M = (C > 0) ? -C / 2 : 0;
									var I = Q - G;
									var D = (I > 0) ? -I / 2 : 0;
									J.marginLeft = M + "px";
					...
					_widthRequiredToFit : function() {
						var C = this._textElement.offsetWidth;
						var B = 0;
						if (this._imagePosition != DC.PushButton.IMAGE_POSITION_NONE
								&& (this._customImageLoaded || this._customImagePressedLoaded)) {
							var A = this._pressed ? this._customImagePressed.width
							...
							case DC.PushButton.IMAGE_POSITION_RIGHT:
								if (this._imageElement.style.marginLeft) {
									B += parseInt(
											this._imageElement.style.marginLeft,
											10)
								}
								if (this._imageElement.style.marginRight) {
									B += parseInt(
											this._imageElement.style.marginRight,
											10)
								}
								B += C + A;
								break;
						...
						B += this._containerLeft + this._containerRight;
						return B
					},
					_setNoBackground : function(A) {
						this._noBackground = A
					}
				});
...
DC.List = Class
		.create(
				DC.ListView,
				{
					...
					maskedBindings : [ "content", "selectionIndexes",
							"selectedIndex", "selectedObject" ],
					...
					partSetup : function(spec) {
						...
						this
								.setAllowsEmptySelection(spec.allowsEmptySelection || false);
						this
								.setAllowsMultipleSelection(spec.allowsMultipleSelection || false);
						this._labelElementId = spec.labelElementId || "label";
						this
								.setUseAlternatingRows(spec.useAlternatingRows || false);
					...
					observeDataArrayChange : function(B) {
						if (B.changeType == DC.ChangeType.setting) {
							var A = this._builtInArrayController(true);
							if (A) {
								if (!A.bindings.arrangedObjects) {
									if (A.bindings.content) {
										A.bindings.content.unbind()
									}
									this.bindNameToKeyPath("content",
											"*.arrangedObjects",
											this._builtInAC)
								}
								A.setValueForKey(B.newValue, "content")
							}
						}
					},
					setDataArray : function(A) {
					...
					setSelectionEnabled : function(B) {
						this._selectionEnabled = B;
						var A = this._builtInArrayController(false);
						if (A) {
							A.setAvoidsEmptySelection(!this.allowsEmpty
					...
					setAllowsMultipleSelection : function(A) {
						this.multiple = A
					},
					getAllowsEmptySelection : function() {
						return this.allowsEmpty
					},
					setAllowsEmptySelection : function(B) {
						this.allowsEmpty = B;
						var A = this._builtInArrayController(false);
						if (A) {
							A.setAvoidsEmptySelection(!this.allowsEmpty
					...
					setUseAlternatingRows : function(A) {
						this._useAlternatingRows = A;
						if (A) {
							Element.addClassName(this.viewElement(),
									"useAlternatingRows")
						} else {
					...
					reloadData : function() {
						if (!this._builtInAC && this.bindings
								&& this.bindings.content) {
							return
						}
						...
						var C = this._builtInArrayController(true);
						var E = DC.KVO.adaptTree( []);
						if (!C) {
							console
									.error("error generating built in array controller");
							return
						}
						if (!C.bindings.content) {
							if (C.bindings.arrangedObjects) {
								C.bindings.arrangedObjects.unbind()
							}
							this.bindNameToKeyPath("content",
									"*.arrangedObjects", C)
						}
						C.setValueForKey(E, "content");
						if (this._useDataSource && dashcode.inDesign) {
							for ( var G = 0; G < this._sampleRows; G++) {
								E.addObject(DC.KVO.adapt( {
									index :G
								}))
							}
						} else {
							if (this.dataArray) {
								for ( var G = 0; G < this.dataArray.length; G++) {
									E.addObject(this.dataArray[G])
								}
							} else {
								if (this._useDataSource && this.dataSource) {
									var B = this.dataSource.numberOfRows();
									for ( var G = 0; G < B; G++) {
										var F = null;
										if (this.dataSource.objectForRow) {
											F = this.dataSource
													.representationForRow(G);
											if (F && DC.typeOf(F) == "object") {
												DC.KVO.adaptTree(F)
											}
										}
										E.addObject(F)
						...
						if (!dashcode.inDesign && E && E.length
								&& this.getSelectionEnabled()
								&& !this.getAllowsEmptySelection()) {
							if (C.selectionIndex() == -1) {
								C.setSelectionIndex(0)
							}
						}
						if (dashcode.inDesign) {
							var A = this;
							setTimeout( function() {
								if (A.viewElement()) {
									A.viewElement().style.display = "block"
								}
							}, 0)
						} else {
							this.viewElement().style.display = "block"
						...
						if (typeof (AppleScrollArea) != "undefined") {
							...
							while (D && D != document) {
								if (D.object
										&& D.object.constructor == AppleScrollArea) {
									D.object.refresh();
									break
								}
								D = D.parentNode
					...
					createElement : function(B, D, A, C) {
						var E = this.base(B, D, A, C);
						if (dashcode.inDesign && (C == 0)
								&& (E != this.templateElement)) {
							A = A || this.container();
							A.insertBefore(this.templateElement, A.firstChild);
							A.removeChild(E);
							if (E.parentNode) {
								E.parentNode.removeChild(E)
							}
							this.templateElement.objectValue = E.objectValue;
							E = this.templateElement;
							DC.View.createViewsForNodeTree(E, null,
									this.__context);
							this.elementUsedForIndex(E, C)
						}
						return E
					},
					highlightSelectionIndexes : function(A) {
						if (dashcode.inDesign) {
							return
						}
						this.base(A)
					},
					elementUsedForIndex : function(D, C) {
						if (C == -1) {
							this._setRowHighlighted(D, false, false);
							this.rows.removeObject(D);
							return
						} else {
							while (C >= this.rows.length) {
						...
						if (this._listStyle == DC.List.ROUNDED_RECTANGLE) {
							D.style.borderTopWidth = C > 0 ? "1px" : "0px"
						}
						if (this._useDataSource) {
							if (this.dataSource && this.dataSource.prepareRow) {
								this.dataSource.prepareRow(D, C,
										D.object.templateElements)
							}
						} else {
							if (this.dataArray) {
								if (D != this.templateRowElement && D.object) {
									D.object.index = C;
									D.object.value = this.dataArray[C]
								}
								this._prepareStaticRow(D, C,
										D.object.templateElements)
							}
						}
						if (D.onclick) {
							var B = this;
							var A = null;
							D.object.clickHandler = D.onclick;
							if (D != this.templateRowElement) {
								D.onclick = null
							}
							D.style.webkitTapHighlightColor = "rgba(0,0,0,0)";
							D.style.webkitTouchCallout = "none"
						}
					},
					setSelectionIndexes : function(A, C) {
						var B = this._selectionIndexes;
						...
						this._animateHighlight = C;
						this.base(A);
						delete this._animateHighlight
					},
					highlightSelection : function(B, A) {
						this.base(B, A);
						this._setRowHighlighted(B, A, this._animateHighlight)
					},
					...
					_findRow : function(B) {
						...
						var A = B.parentNode;
						if (this.viewElement() == A) {
							return B
						}
						return this._findRow(A)
					},
					onmousedown : function(B) {
						if (DC.Support.Touches) {
							var A = this._touchedRow;
							if (!A) {
								A = this._findRow(B.target ? B.target
										: B.srcElement)
							}
							if (A && !A.object._highlighted
									&& this._selectionEnabled) {
								this._setRowHighlighted(A, true, false);
								this._temporaryHighlight = A
							}
						}
						this.base(B)
					},
					onmouseup : function(A) {
						if (DC.Support.Touches) {
							if (this._temporaryHighlight) {
								this._setRowHighlighted(
										this._temporaryHighlight, false, false);
					...
					onclick : function(B) {
						var A = this._temporaryHighlight;
						this.base(B);
						if (!this.action) {
							if (!A) {
								A = this._findRow(B.target ? B.target
							...
							if (A && A.object && A.object.clickHandler) {
								this._sendLegacyOnClick(B, A,
										A.object.clickHandler)
							}
						}
						delete this._temporaryHighlight
					},
					onswipe : function(B) {
						if (this.dataSource && this.dataSource.rowSwiped) {
							var A = this._findRow(B.target ? B.target
									: B.srcElement);
							this.dataSource.rowSwiped(A, B, 5)
						}
					},
					_prepareStaticRow : function(E, H, I) {
						var F = null;
						var D = this.dataArray[H];
						if (dashcode.inDesign && !I) {
							F = document.getElementById(this._labelElementId)
						}
						for ( var J in I) {
							if (this._labelElementId
									&& this._labelElementId.length > 0
									&& J.indexOf(this._labelElementId) == 0) {
								F = I[J];
								break
							}
						}
						if (this._labelElementId && this._labelElementId.length
								&& (E == this.templateElement)) {
							var G = Element.queryAll(E, "#"
									+ this._labelElementId);
							if (G.length > 0) {
								F = G[0]
							}
						}
						...
						if (F) {
							var B = dashcode.getLocalizedString(A);
							if (B) {
								A = B
							}
							F.innerHTML = A
					...
					_builtInArrayController : function(A) {
						if (!("_builtInAC" in this)) {
							this._builtInAC = new DC.ArrayController();
							this._builtInAC.selectsInsertedObjects = false;
							this._builtInAC.setAvoidsEmptySelection(!this
									.getAllowsEmptySelection()
									&& this.getSelectionEnabled()
									&& !dashcode.inDesign);
							this.bindNameToKeyPath("content",
									"*.arrangedObjects", this._builtInAC);
							this.bindNameToKeyPath("selectionIndexes",
									"*.selectionIndexes", this._builtInAC);
							if (this.id) {
					...
					_setDataArray : function(A) {
						this.dataArray = A;
						this.reloadData()
					},
					_setSampleRows : function(A) {
						this._sampleRows = A;
						this.reloadData()
					},
					_setUseDataSource : function(A) {
						this._useDataSource = A;
						this.reloadData()
					},
					_setLabelElementId : function(A) {
						this._labelElementId = A;
						this.reloadData()
					},
					_setRowHighlighted : function(H, D, G) {
						if (!H || !H.object) {
							return
						}
						if (D != H.object._highlighted) {
							if (D) {
								if (this._rowHighlight) {
									if (!H.object.rowHighlight) {
										H.object.rowHighlight = this._rowHighlight
												.cloneNode(true);
										var I = H.object.index;
										if (this._listStyle == DC.List.ROUNDED_RECTANGLE
												&& (I == 0 || I == H.parentNode.children.length - 1)) {
											var F = Element
													.getStyles(
															this.viewElement(),
															[
											...
											var E = H.object.rowHighlight.style;
											if (I == 0) {
												var A = parseInt(
														F["border-top-width"],
														10);
												E.webkitBorderTopLeftRadius = (parseInt(
														F["-webkit-border-top-left-radius"],
														10) - A)
														+ "px";
												E.webkitBorderTopRightRadius = (parseInt(
														F["-webkit-border-top-right-radius"],
														10) - A)
														+ "px"
											}
											if (I == H.parentNode.children.length - 1) {
												var A = parseInt(
														F["border-bottom-width"],
														10);
												E.webkitBorderBottomLeftRadius = (parseInt(
														F["-webkit-border-bottom-left-radius"],
														10) - A)
														+ "px";
												E.webkitBorderBottomRightRadius = (parseInt(
									...
									var C = H.object.rowHighlight;
									C.style.opacity = 0;
									H.insertBefore(C, H.firstChild);
									if (G) {
										C.style.webkitTransitionProperty = "opacity";
										C.style.webkitTransitionDuration = ".25s";
										window.setTimeout( function() {
											C.style.opacity = 1
										}, 0)
									} else {
										C.style.webkitTransitionProperty = "none";
										C.style.webkitTransitionDuration = "0s";
										C.style.opacity = 1
									}
									Element.addClassName(H,
											DC.Style.kSelectedClass)
								}
							} else {
								var C = H.object.rowHighlight;
								if (C) {
									if (G) {
										...
										Event.observe(C, "webkitTransitionEnd",
												B);
										window.setTimeout( function() {
											C.style.opacity = 0
										}, 0)
									...
									Element.removeClassName(H,
											DC.Style.kSelectedClass)
								}
							}
							H.object._highlighted = D
						}
					}
				});
...
DC.ActivityIndicator = Class
		.create(
				DC.DashcodePart,
				{
					...
					partSetup : function(A) {
						var B = this.viewElement();
						B.style.webkitAnimationName = "none";
						B.style.webkitAnimationDuration = "1s";
					...
					setAnimating : function(A) {
						this.viewElement().style.webkitAnimationName = (A ? "dashcode-activity-indicator"
								: "none")
					},
					animating : function() {
