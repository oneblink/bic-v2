// files from AJAXSLT assembed in order: xmltoken.js, util.js, dom.js, xpath.js, xslt,js
...
function xmlParse(xml) {
  ...
  if (xml.match(/^<\?xml/)) {
    ...
    if (xml.search(new RegExp(XML10_VERSION_INFO)) == 5) {
      ...
      console.log('VersionInfo is missing, or unknown version number.');
    }
  } else {
...
StepExpr.prototype.evaluate = function(ctx) {
  ...
  if (this.axis ==  xpathAxis.ANCESTOR_OR_SELF) {
    ...
    console.log('not implemented: axis namespace');
...
FunctionCallExpr.prototype.xpathfunctions = {
  ...
  'local-name': function(ctx) {
    console.log('not implmented yet: XPath function local-name()');
  },

  'namespace-uri': function(ctx) {
    console.log('not implmented yet: XPath function namespace-uri()');
  },
...
BinaryExpr.prototype.evaluate = function(ctx) {
  ...
  switch (this.op.value) {
    ...
    default:
      console.log('BinaryExpr.evaluate: ' + this.op.value);
  }
  return ret;
};
...
function xsltProcessContext(input, template, output) {
  ...
  if (nodename.length == 1 || nodename[0] != 'xsl') {
    ...
    switch(nodename[1]) {
    case 'apply-imports':
      console.log('not implemented: ' + nodename[1]);
      break;
    ...
    case 'otherwise':
      console.log('error if here: ' + nodename[1]);
      break;
