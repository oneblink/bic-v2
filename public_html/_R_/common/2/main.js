...
//
  ...
  $(document.body).bind('taskBegun', onTaskBegun);
  $(document.body).bind('taskComplete', onTaskComplete);
}
...
function populateKeywordList(category) {
	MyAnswers.log('populateKeywordList(1a):category=' +  category + '; hasCategories=' + hasCategories);
	var width;
...
{
	MyAnswers.log("getSiteConfig(0a): typeof(siteConfig) => " + typeof(siteConfig));
...
function processSiteConfig()
{
	try {
		...
		hasCategories = siteConfig.categories_config != 'no' && siteConfig.categories.length > 0;
		...
		ajaxQueue.add({
		 ...
		 complete: function(xhr, textstatus) { // readystate == 4
			 runListWithDelay([
				 ...
				 100,
				 function() {
					 //MyAnswers.log("showAnswerView(): list complete");
					 return true;
				 });
		 },
...
function setupForms(view)
{
	...
	var firstColumnWidth = form.find('.bForm-input').first().siblings().first().width();
...
function setSubmitCachedFormButton() {
  ...
  if (deviceVars.device === 'iphone' || deviceVars.device === 'iphone_pg')
		setupParts();
}
		...
		complete: function(xhr, textstatus) { // readystate == 4
			...
			if (currentBox.attr('id').indexOf('answerBox') !== -1)
			{
				currentBox.hide('slide', { direction: 'left'}, 300, function() {
					runListWithDelay([(function() { MyAnswers.log("SubmitWithRetry(1):"); insertHTML(currentBox[0], html); return true; }),
														(function() { currentBox.show('slide', { direction: 'right'}, 300); return true; }),
			...
			{
				runListWithDelay([(function() { 
														MyAnswers.log("SubmitWithRetry(2):"); 
														prepareSecondLevelAnswerViewForDevice();
														addBackHistory("");
														return true;
													}),
													(function() { insertHTML(answerBox2, html); return true; }),
													(function() { setCurrentView('answerView2', false, true); return true; })],
													400,
													function() {
														setSubmitCachedFormButton();
														$(document.body).trigger('taskComplete');
...
function submitAction(keyword, action) {
	...
	ajaxQueue.add({
		...
		data: requestData,
		beforeSend: function(xhr) {
		  MyAnswers.log("GetAnswer transaction GET: " + requestUrl + "?" + requestData);
			httpAnswerRequest = xhr;
			$(document.body).trigger('taskBegun');
		},
		complete: function(xhr, textstatus) { // readystate == 4
...
function setupGoogleMaps()
{
	$('div.googlemap').each(function(index, element) {
		...
		if (isLocationAvailable())
		{
			...
			$(document.body).bind('locationUpdated', function() {
				currentMarker.setPosition(new google.maps.LatLng(latitude, longitude));
			});
...
function setupGoogleMapsDirections(element, data, map)
{
	...
	MyAnswers.log(directionsOptions);
	var mapOptions = {
		mapTypeId: google.maps.MapTypeId[data.type.toUpperCase()]
	};
	MyAnswers.log(mapOptions);
	map.setOptions(mapOptions);
	var directionsDisplay = new google.maps.DirectionsRenderer();
	directionsDisplay.setMap(map);
	MyAnswers.log($(element).next('.googledirections')[0]);
		...
		if (typeof(navigator.geolocation) != 'undefined')
		{
			navigator.geolocation.watchPosition(function(position) {
				if (latitude != position.coords.latitude || longitude != position.coords.longitude)
				{
					...
					$(document.body).trigger('locationUpdated');
				}
...
{
	MyAnswers.log(typeof(window.DOMParser));
	var xml;
