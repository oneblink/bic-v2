...
function xmlParse(a){var b=/\/$/,c,d;if(a.match(/^<\?xml/))if(a.search(RegExp(XML10_VERSION_INFO))==5){c=XML10_TAGNAME_REGEXP;d=XML10_ATTRIBUTE_REGEXP}else if(a.search(RegExp(XML11_VERSION_INFO))==5){c=XML11_TAGNAME_REGEXP;d=XML11_ATTRIBUTE_REGEXP}else Myanswers.log("AJAXSLT: VersionInfo is missing, or unknown version number.");else{c=XML10_TAGNAME_REGEXP;d=XML10_ATTRIBUTE_REGEXP}var e=new XDocument,f=[],g=e;f.push(g);var k="";a=stringSplit(a,"<");for(var h=1;h<a.length;++h){var i=stringSplit(a[h],
">"),j=i[0];i=xmlResolveEntities(i[1]||"");if(k){j=a[h].indexOf(k);if(j!=-1){var m=a[h].substring(0,j);g.nodeValue+="<"+m;f.pop();g=f[f.length-1];i=a[h].substring(j+k.length);k=""}else{g.nodeValue+="<"+a[h];i=null}}else if(j.indexOf("![CDATA[")==0){m=8;j=a[h].indexOf("]]\>");if(j!=-1){m=a[h].substring(m,j);var l=domCreateCDATASection(e,m);domAppendChild(g,l)}else{m=a[h].substring(m);i=null;l=domCreateCDATASection(e,m);domAppendChild(g,l);g=l;f.push(l);k="]]\>"}}else if(j.indexOf("!--")==0){m=3;j=
a[h].indexOf("--\>");if(j!=-1){m=a[h].substring(m,j);l=domCreateComment(e,m);domAppendChild(g,l)}else{m=a[h].substring(m);i=null;l=domCreateComment(e,m);domAppendChild(g,l);g=l;f.push(l);k="--\>"}}else if(j.charAt(0)=="/"){f.pop();g=f[f.length-1]}else if(j.charAt(0)!="?")if(j.charAt(0)!="!"){m=j.match(b);l=c.exec(j)[1];l=domCreateElement(e,l);for(var n;n=d.exec(j);){var o=xmlResolveEntities(n[5]||n[7]||"");domSetAttribute(l,n[1],o)}domAppendChild(g,l);if(!m){g=l;f.push(l)}}i&&g!=e&&domAppendChild(g,
domCreateTextNode(e,i))}return e}var DOM_ELEMENT_NODE=1,DOM_ATTRIBUTE_NODE=2,DOM_TEXT_NODE=3,DOM_CDATA_SECTION_NODE=4,DOM_ENTITY_REFERENCE_NODE=5,DOM_ENTITY_NODE=6,DOM_PROCESSING_INSTRUCTION_NODE=7,DOM_COMMENT_NODE=8,DOM_DOCUMENT_NODE=9,DOM_DOCUMENT_TYPE_NODE=10,DOM_DOCUMENT_FRAGMENT_NODE=11,DOM_NOTATION_NODE=12;
...
xmlValue(e[b]).split(/\s+/),g=0;g<f.length;++g)d.push(f[g])}else d=b.stringValue().split(/\s+/);a=a.root;for(b=0;b<d.length;++b)(e=a.getElementById(d[b]))&&c.push(e);return new NodeSetValue(c)},"local-name":function(){Myanswers.log("AJAXSLT: not implmented yet: XPath function local-name()")},"namespace-uri":function(){Myanswers.log("AJAXSLT: not implmented yet: XPath function namespace-uri()")},name:function(a){assert(this.args.length==1||this.args.length==0);a=this.args.length==0?[a.node]:this.args[0].evaluate(a).nodeSetValue();
...
d});break;case "<=":b=this.compare(a,function(c,d){return c<=d});break;case ">":b=this.compare(a,function(c,d){return c>d});break;case ">=":b=this.compare(a,function(c,d){return c>=d});break;default:Myanswers.log("AJAXSLT: BinaryExpr.evaluate: "+this.op.value)}return b};
...
function xpathSortByKey(a,b){for(var c=0;c<a.key.length;++c){var d=a.key[c].order=="descending"?-1:1;if(a.key[c].value>b.key[c].value)return+1*d;else if(a.key[c].value<b.key[c].value)return-1*d}return 0}function xpathEval(a,b){return xpathParse(a).evaluate(b)}function xsltProcess(a,b){var c=domCreateDocumentFragment(new XDocument);xsltProcessContext(new ExprContext(a),b,c);return xmlText(c)}
function xsltProcessContext(a,b,c){var d=xmlOwnerDocument(c),e=b.nodeName.split(/:/);if(e.length==1||e[0]!="xsl")xsltPassThrough(a,b,c,d);else switch(e[1]){case "apply-imports":Myanswers.log("AJAXSLT: not implemented: "+e[1]);break;case "apply-templates":e=(e=xmlGetAttribute(b,"select"))?xpathEval(e,a).nodeSetValue():a.node.childNodes;a=a.clone(e[0],0,e);xsltWithParam(a,b);xsltSort(a,b);var f=xmlGetAttribute(b,"mode");d=b.ownerDocument.documentElement;e=[];for(b=0;b<d.childNodes.length;++b){var g=
d.childNodes[b];g.nodeType==DOM_ELEMENT_NODE&&g.nodeName=="xsl:template"&&g.getAttribute("mode")==f&&e.push(g)}for(d=0;d<a.contextSize();++d){g=a.nodelist[d];for(b=0;b<e.length;++b)xsltProcessContext(a.clone(g,d),e[b],c)}break;case "attribute":e=xmlGetAttribute(b,"name");e=xsltAttributeValue(e,a);g=domCreateDocumentFragment(d);xsltChildNodes(a,b,g);b=xmlValue(g);domSetAttribute(c,e,b);break;case "attribute-set":Myanswers.log("AJAXSLT: not implemented: "+e[1]);break;case "call-template":e=xmlGetAttribute(b,
"name");d=b.ownerDocument.documentElement;a=a.clone();xsltWithParam(a,b);for(b=0;b<d.childNodes.length;++b){g=d.childNodes[b];if(g.nodeType==DOM_ELEMENT_NODE&&g.nodeName=="xsl:template"&&domGetAttribute(g,"name")==e){xsltChildNodes(a,g,c);break}}break;case "choose":xsltChoose(a,b,c);break;case "comment":g=domCreateDocumentFragment(d);xsltChildNodes(a,b,g);b=xmlValue(g);b=domCreateComment(d,b);c.appendChild(b);break;case "copy":(g=xsltCopy(c,a.node,d))&&xsltChildNodes(a,b,g);break;case "copy-of":e=
xmlGetAttribute(b,"select");b=xpathEval(e,a);if(b.type=="node-set"){e=b.nodeSetValue();for(b=0;b<e.length;++b)xsltCopyOf(c,e[b],d)}else{g=domCreateTextNode(d,b.stringValue());domAppendChild(c,g)}break;case "decimal-format":Myanswers.log("AJAXSLT: not implemented: "+e[1]);break;case "element":e=xmlGetAttribute(b,"name");e=xsltAttributeValue(e,a);g=domCreateElement(d,e);domAppendChild(c,g);xsltChildNodes(a,b,g);break;case "fallback":Myanswers.log("AJAXSLT: not implemented: "+e[1]);break;case "for-each":xsltForEach(a,
b,c);break;case "if":d=xmlGetAttribute(b,"test");xpathEval(d,a).booleanValue()&&xsltChildNodes(a,b,c);break;case "import":Myanswers.log("AJAXSLT: not implemented: "+e[1]);break;case "include":Myanswers.log("AJAXSLT: not implemented: "+e[1]);break;case "key":Myanswers.log("AJAXSLT: not implemented: "+e[1]);break;case "message":Myanswers.log("AJAXSLT: not implemented: "+e[1]);break;case "namespace-alias":Myanswers.log("AJAXSLT: not implemented: "+e[1]);break;case "number":Myanswers.log("AJAXSLT: not implemented: "+
e[1]);break;case "otherwise":Myanswers.log("AJAXSLT: error if here: "+e[1]);break;case "output":break;case "preserve-space":Myanswers.log("AJAXSLT: not implemented: "+e[1]);break;case "processing-instruction":Myanswers.log("AJAXSLT: not implemented: "+e[1]);break;case "sort":break;case "strip-space":Myanswers.log("AJAXSLT: not implemented: "+e[1]);break;case "stylesheet":case "transform":xsltChildNodes(a,b,c);break;case "template":(d=xmlGetAttribute(b,"match"))&&xsltMatch(d,a)&&xsltChildNodes(a,b,
c);break;case "text":b=xmlValue(b);g=domCreateTextNode(d,b);c.appendChild(g);break;case "value-of":e=xmlGetAttribute(b,"select");b=xpathEval(e,a).stringValue();g=domCreateTextNode(d,b);c.appendChild(g);break;case "param":xsltVariable(a,b,false);break;case "variable":xsltVariable(a,b,true);break;case "when":Myanswers.log("AJAXSLT: error if here: "+e[1]);break;case "with-param":Myanswers.log("AJAXSLT: error if here: "+e[1]);break;default:Myanswers.log("AJAXSLT: error if here: "+e[1]);break}}
function xsltWithParam(a,b){for(var c=0;c<b.childNodes.length;++c){var d=b.childNodes[c];d.nodeType==DOM_ELEMENT_NODE&&d.nodeName=="xsl:with-param"&&xsltVariable(a,d,true)}}
...
function xsltPassText(a){if(!a.nodeValue.match(/^\s*$/))return true;a=a.parentNode;if(a.nodeName=="xsl:text")return true;for(;a&&a.nodeType==DOM_ELEMENT_NODE;){var b=domGetAttribute(a,"xml:space");if(b)if(b=="default")return false;else if(b=="preserve")return true;a=a.parentNode}return false}
