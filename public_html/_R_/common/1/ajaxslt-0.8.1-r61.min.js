...
g.nodeType==DOM_ELEMENT_NODE&&g.nodeName=="xsl:template"&&g.getAttribute("mode")==f&&e.push(g)}for(d=0;d<a.contextSize();++d){g=a.nodelist[d];for(b=0;b<e.length;++b)xsltProcessContext(a.clone(g,d),e[b],c)}break;case "attribute":e=xmlGetAttribute(b,"name");e=xsltAttributeValue(e,a);g=domCreateDocumentFragment(d);xsltChildNodes(a,b,g);b=xmlValue(g);domSetAttribute(c,e,b);break;case "attribute-set":console.log("not implemented: "+e[1]);break;case "call-template":e=xmlGetAttribute(b,"name");d=b.ownerDocument.documentElement;
a=a.clone();xsltWithParam(a,b);for(b=0;b<d.childNodes.length;++b){g=d.childNodes[b];if(g.nodeType==DOM_ELEMENT_NODE&&g.nodeName=="xsl:template"&&domGetAttribute(g,"name")==e){xsltChildNodes(a,g,c);break}}break;case "choose":xsltChoose(a,b,c);break;case "comment":g=domCreateDocumentFragment(d);xsltChildNodes(a,b,g);b=xmlValue(g);b=domCreateComment(d,b);c.appendChild(b);break;case "copy":(g=xsltCopy(c,a.node,d))&&xsltChildNodes(a,b,g);break;case "copy-of":e=xmlGetAttribute(b,"select");b=xpathEval(e,
a);if(b.type=="node-set"){e=b.nodeSetValue();for(b=0;b<e.length;++b)xsltCopyOf(c,e[b],d)}else{g=domCreateTextNode(d,b.stringValue());domAppendChild(c,g)}break;case "decimal-format":console.log("not implemented: "+e[1]);break;case "element":e=xmlGetAttribute(b,"name");e=xsltAttributeValue(e,a);g=domCreateElement(d,e);domAppendChild(c,g);xsltChildNodes(a,b,g);break;case "fallback":console.log("not implemented: "+e[1]);break;case "for-each":xsltForEach(a,b,c);break;case "if":d=xmlGetAttribute(b,"test");
