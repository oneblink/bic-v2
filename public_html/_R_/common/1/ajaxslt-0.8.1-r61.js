...
function xmlParse(xml) {
  ...
  if (xml.match(/^<\?xml/)) {
    ...
    if (xml.search(new RegExp(XML10_VERSION_INFO)) == 5) {
      ...
      Myanswers.log('AJAXSLT: VersionInfo is missing, or unknown version number.');
    }
  } else {
...
StepExpr.prototype.evaluate = function(ctx) {
  ...
  if (this.axis ==  xpathAxis.ANCESTOR_OR_SELF) {
    ...
    Myanswers.log('AJAXSLT: not implemented: axis namespace');
...
FunctionCallExpr.prototype.xpathfunctions = {
  ...
  'local-name': function(ctx) {
    Myanswers.log('AJAXSLT: not implmented yet: XPath function local-name()');
  },

  'namespace-uri': function(ctx) {
    Myanswers.log('AJAXSLT: not implmented yet: XPath function namespace-uri()');
  },
...
BinaryExpr.prototype.evaluate = function(ctx) {
  ...
  switch (this.op.value) {
    ...
    default:
      Myanswers.log('AJAXSLT: BinaryExpr.evaluate: ' + this.op.value);
  }
  return ret;
};
...
function xsltProcessContext(input, template, output) {
  ...
  if (nodename.length == 1 || nodename[0] != 'xsl') {
    ...
    switch(nodename[1]) {
    case 'apply-imports':
      Myanswers.log('AJAXSLT: not implemented: ' + nodename[1]);
      break;
    ...
    case 'otherwise':
      Myanswers.log('AJAXSLT: error if here: ' + nodename[1]);
      break;
