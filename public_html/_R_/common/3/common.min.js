...
return i}})();
(function(e,f){function k(a){switch(a){case u:case v:case s:case w:case x:return true;default:return false}}function i(a,b){return!a?false:a.constructor.toString().match(RegExp(b+"\\(\\)","i"))!==null}function l(a,b,d){if(i(a,"Function"))return a.apply(b||c,i(d,"Array")?d:[d])}function m(a){if(a===undefined)return"";return JSON.stringify(a)}function h(a){return F.test(a)?JSON.parse(a):a}function p(a,b,d){if(!k(a))throw G;if(d[a]!==undefined)throw A;if(l(d)===true){c.available[a]=true;b.flavor=a;B[a]=
H.extend(b)}else{c.available[a]=false;c.enginePriority=e.map(c.enginePriority,function(g){return g===a?null:g})}}function t(){if(!c.isReady)if(C&&c.isFlashReady||!C){c.isReady=true;c.trigger("jstore-ready",[q[n]])}}function z(){if(navigator.plugins&&navigator.plugins.length>0)try{if(navigator.mimeTypes["application/x-shockwave-flash"].enabledPlugin)return(navigator.plugins["Shockwave Flash 2.0"]||navigator.plugins["Shockwave Flash"]).description.replace(/\D+/g,",").match(/^,?(.+),?$/)[1]}catch(a){}else if(typeof f.ActiveXObject!=
"undefined"){try{var b=new ActiveXObject("ShockwaveFlash.ShockwaveFlash.6");try{b.AllowScriptAccess="always"}catch(d){return"6,0,0"}}catch(g){}return(new ActiveXObject("ShockwaveFlash.ShockwaveFlash")).GetVariable("$version").replace(/\D+/g,",").match(/^,?(.+),?$/)[1]}return"0,0,0"}var A="JSTORE_ENGINE_NAMESPACE_COLLISION",G="JSTORE_INVALID_FLAVOR",F;F=/^("(\\.|[^"\\\n\r])*?"|[,:{}\[\]0-9.\-+Eaeflnr-u \n\r\t])+?$/;var u="jstore-html5-local",v="jstore-html5-sql",s="jstore-flash",w="jstore-google-gears",
x="jstore-msie",H,c={},B={},q={},C=false,o={},r={project:undefined,flash:"jStore.Flash.html",json:"browser.json.js"},n;e.extend(c,{enginePriority:[v,w,u,s,x],available:{},isReady:false,isFlashReady:false,flavors:{local:u,sql:v,flash:s,gears:w,msie:x},init:function(a,b,d){e.extend(r,{project:a},b);e(function(){r.errorCallback&&c.error(r.errorCallback);f.JSON===undefined&&e.ajax({url:r.json,complete:e.noop(),type:"GET",dataType:"script",cache:false});try{d!==undefined?c.create(d,a,"default"):c.create(c.enginePriority[0],
